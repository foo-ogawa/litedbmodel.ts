...
export class Conditions<Model> {
  ...
  addSql<V>(fragment: SqlTypedFragment<V, Model>, value?: V): this {
    if (value !== undefined) {
      this.conds.push([fragment, value] as unknown as Cond);
    } else {
      // Value-free condition (IS NULL, etc.) — push as direct SqlTypedFragment
      this.conds.push(fragment as unknown as CondElement);
    }
    return this;
  }
...
export type Cond = 
  ...
  | readonly [SqlTypedFragment<any, any>, unknown]  // sql tag tuple: [sql`${Col} > ?`, value]
...
export type CondOf<M> = 
  ...
  | readonly [SqlTypedFragment<any, ModelInstance<M>>, unknown]  // Pattern A: sql tag tuple
  | readonly [string, unknown]
  | readonly [string];
 ...
 * @internal
 */
export type CondElement = Cond | OrCond | SkipType | SqlCondition | SqlTypedFragment | SqlFragment;
 ...
 * @internal
 */
export type CondElementOf<M> =
  | CondOf<M>
  | OrCondOf<M>
  | SkipType
  | SqlCondition<ModelInstance<M>>    // Pattern B / value-free condition
  | SqlTypedFragment<any, ModelInstance<M>>  // Value-free condition (IS NULL)
  | SqlFragment;                      // General sql fragment as condition
...
function condToKeyValue(cond: Cond): [string, unknown] | null {
  ...
  // Handle SqlTypedFragment tuple: [sql`${Col} > ?`, value]
  if (isSqlTypedFragment(keyOrCol)) {
    return [keyOrCol.sql, value];
  }
  ...
  const col = keyOrCol as Column<any, any>;
 ...
 */
/**
 * Convert a SqlCondition/SqlFragment/SqlTypedFragment to key-value pair(s).
 * @internal
 */
function sqlFragmentToKeyValue(
  frag: SqlCondition | SqlFragment | SqlTypedFragment
): [string, unknown] | null {
  if (frag.params.length === 0) {
    // Value-free condition (e.g., IS NULL)
    return [frag.sql, true];
  }
  if (frag.params.length === 1) {
    return [frag.sql, frag.params[0]];
  }
  // Multiple params (e.g., BETWEEN ? AND ?)
...
export function condsToRecord(
  ...
  for (const cond of conditions) {
    ...
    if (isOrCond(cond)) {
      ...
      const kv = sqlFragmentToKeyValue(cond);
      if (kv !== null) {
        const [key, value] = kv;
        result[key] = value;
      }
