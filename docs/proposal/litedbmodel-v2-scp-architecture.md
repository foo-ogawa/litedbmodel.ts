...
- **3つの利用モードを同居**させる（単一意味論、feasibility §2）:
  1. **TS 直接利用（eager）** — 従来通りの公開 API を直接呼ぶ（Native Interpret 経路）。
  2. **SCP 宣言ブロック** — graphddb のように Query/Command を宣言的に定義 → **ビルド時に事前コンパイル**して IR（dialect SQL + 動的 condition の fragment）/ TS コードへ変換。
  3. **多言語利用** — publish された宣言ブロック（IR）を各言語 runtime から呼ぶ。
...
| **Authoring Parse** | 公開 API（`User.find(...)`）と SCP 宣言（`defineQuery`/`defineCommand`）を **Component-graph IR** へ落とす |
...
従来どおり公開 API を**直接**呼ぶ。内部では Authoring Parse → IR → Handler（Native Interpret 経路、feasibility §2）で即時実行される。書込は write-time relations があれば自動で1トランザクションに束ねられる（§6）。
...
### 2.4 SCP 宣言ブロック（ビルド時事前コンパイル → IR/TS コード）

graphddb の `publishQuery`/`publishCommand` と同型の宣言ブロックを定義できる。**ビルド時に事前コンパイル**され、**IR（dialect SQL + 動的 condition の fragment）**および（任意で）TS コードへ変換される。

```ts
// 宣言ブロック（公開契約 = CQRS 境界）
export const PostQueries = litedbmodel.defineQuery({
  search: {
    from: Post,
    where: ($) => [
      Post.author_id.eq($.authorId),
      $.status ? Post.status.eq($.status) : SKIP,   // SKIP → fragment の存在規則
      Post.created_at.gte($.since),
    ],
    with: { author: { select: ['id', 'name'] } },   // relation は staged 合成
...
export const PostCommands = litedbmodel.defineCommand({
  create: {
    create: Post,                    // Post.writes.create の write-time relations を導出
    input: { author_id: p.number(), title: p.string(), request_id: p.string() },
    result: { select: ['id', 'title'] },
  },
});
```

- **公開されるのは use-case メソッド名**（`search` / `create`）のみ。SQL 種別（SELECT/INSERT）は runtime 不可視（graphddb と同型）。
- 入力の arity（単数/配列）・cardinality（one/many）は**導出され型で強制**（N+1 型安全）。
...
公開境界        = CQRS Contract（Query / Command）※ 多言語で共有
  ...
  └─ SCP 宣言ブロック ……… defineQuery/defineCommand → Component-graph IR（AOT）
...
**Command 導出（graphddb mutation-derivation 同型）:** `defineCommand({ create: { create: Post, ... } })` の宣言的 intent から、コンパイラが `Post.writes.create` を展開して**順序付き多文トランザクション**を導出:
...
`defineQuery`/`defineCommand` の本体は **SCP の Behavior**として扱われ、bc のコンパイルパイプラインで Component-graph IR へ落ちる:
...
bc の3経路をそのまま持つ（利用者要件 2.3〜2.5）:

1. **Native Interpret（TS のみ）** — 宣言/公開 API を呼ぶと即 plan → 実行。dev UX・eager。
2. **IR Reference, Dynamic（全言語）** — publish された IR(JSON) を薄い runtime が `runPlan`/`evaluateExpression` で解釈実行。
3. **Codegen, Static（全言語）** — IR → 各言語ソース生成（**ビルド時事前コンパイル**、runtime≈0）。litedbmodel の「ビルド時に TS コード/IR へ変換」はこの経路（bc issue #13 の共有 generator に SQL catalog を供給）。

いずれも同一 Component-graph IR を通るので、eager で書いたロジックの SCP 化・多言語化は「配線を宣言に移す」リファクタで済む（feasibility §2）。
...
2. **Authoring Parse**: 公開 API・`defineQuery`/`defineCommand` → Component-graph IR。
...
1. **v2 α**: **SQLite + TS**・Query 契約のみで縦1本（公開 API/defineQuery → Component-graph IR → Backend Compile → 薄い Runtime）。golden = 同一入力→同一 SQL + 同一結果。動的展開仕様（SKIP/fragment 木）を確定。← bc issue #1 の本体。
