...
| BOOLEAN | `bool` | `bool` | |
| DATE / TIMESTAMP / DATETIME | `date` | `string` | **一旦 string**（オーナー決定）。bc の `date` scalar 新設（behavior-contracts#84）は将来の最適化として **defer**。litedbmodel 列型は `date` のまま（意味は保持）だが bc 表現は string |
