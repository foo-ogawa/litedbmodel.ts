...
// GENERATED by litedbmodel codegen — the RUST COMPANION for `generated_update` (epic #123/#124).
...
impl<'a> HandlerNRUpdate for Rt<'a> {
    type Wire = Wire;
    fn node_n0(&self, ports: &PortsNRUpdateN0, _bound: Option<String>) -> Result<Wire, BehaviorError> {
        let ctx = self.src.ctx().map_err(cvt)?;
        litedbmodel_runtime::exec(&ctx, &ports.f_sql, &[litedbmodel_runtime::wp(&ports.f_p0), litedbmodel_runtime::wp(&ports.f_p1)], litedbmodel_runtime::ExecMode::Summary).map_err(cvt)
    }
}
