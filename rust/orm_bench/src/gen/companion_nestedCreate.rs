...
// GENERATED by litedbmodel codegen — the RUST COMPANION for `generated_nestedCreate` (epic #123/#124).
...
use super::generated_nestedCreate::*;
...
impl<'a, 'b, 'c> HandlerNRNestedCreate for TxRt<'a, 'b, 'c> {
    type Wire = Wire;
    fn node_tx_body_0(&self, ports: &PortsNRNestedCreateTxBody0, _bound: Option<String>) -> Result<Wire, BehaviorError> {
    ...
    fn node_tx_body_1(&self, ports: &PortsNRNestedCreateTxBody1, _bound: Option<String>) -> Result<Wire, BehaviorError> {
...
pub fn run(driver: &dyn Driver, in_: InNRNestedCreate) -> Result<bool, BehaviorError> {
    litedbmodel_runtime::run_transaction(driver, |ctx| run_native_raw_struct_NestedCreate(&TxRt { ctx }, in_)).map_err(cvt)
}
...
pub fn run_on(src: litedbmodel_runtime::ConnSource, connection: Option<&str>, dialect: litedbmodel_runtime::TxDialect, options: &litedbmodel_runtime::TransactionOptions, make_in: impl Fn() -> InNRNestedCreate) -> Result<bool, litedbmodel_runtime::SqlFailure> {
    litedbmodel_runtime::run_transaction_on(src, connection, dialect, options, move |ctx| {
        run_native_raw_struct_NestedCreate(&TxRt { ctx }, make_in()).map_err(|e| litedbmodel_runtime::SqlFailure { kind: e.code, policy: "fail".to_string(), sqlite_code: None, message: e.message })
    })
}
