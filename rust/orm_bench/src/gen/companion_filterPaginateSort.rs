...
// GENERATED by litedbmodel codegen — the RUST COMPANION for `generated_filterPaginateSort` (epic #123/#124).
...
use super::generated_filterPaginateSort::*;
...
impl<'a> HandlerNRFilterPaginateSort for Rt<'a> {
    type Wire = Wire;
    fn node_n0(&self, ports: &PortsNRFilterPaginateSortN0, _bound: Option<String>) -> Result<Wire, BehaviorError> {
        let ctx = self.src.ctx().map_err(cvt)?;
        litedbmodel_runtime::exec(&ctx, &ports.f_sql, &[litedbmodel_runtime::wp(&ports.f_p0), litedbmodel_runtime::wp(&ports.f_p1), litedbmodel_runtime::wp(&ports.f_p2)], litedbmodel_runtime::ExecMode::Rows).map_err(cvt)
    }
}
