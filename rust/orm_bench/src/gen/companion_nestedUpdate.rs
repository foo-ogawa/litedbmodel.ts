...
// GENERATED by litedbmodel codegen — the RUST COMPANION for `generated_nestedUpdate` (epic #123/#124).
...
use super::generated_nestedUpdate::*;
...
impl<'a, 'b, 'c> HandlerNRNestedUpdate for TxRt<'a, 'b, 'c> {
    type Wire = Wire;
    fn node_tx_body_0(&self, ports: &PortsNRNestedUpdateTxBody0, _bound: Option<String>) -> Result<Wire, BehaviorError> {
    ...
    fn node_tx_body_1(&self, ports: &PortsNRNestedUpdateTxBody1, _bound: Option<String>) -> Result<Wire, BehaviorError> {
        litedbmodel_runtime::exec(self.ctx, &ports.f_sql, &[litedbmodel_runtime::wp(&ports.f_p0), litedbmodel_runtime::wp(&ports.f_p1)], litedbmodel_runtime::ExecMode::SummarySingle).map_err(cvt)
    }
}
...
pub fn run(driver: &dyn Driver, in_: InNRNestedUpdate) -> Result<bool, BehaviorError> {
    litedbmodel_runtime::run_transaction(driver, |ctx| run_native_raw_struct_NestedUpdate(&TxRt { ctx }, in_)).map_err(cvt)
}
...
pub fn run_on(src: litedbmodel_runtime::ConnSource, connection: Option<&str>, dialect: litedbmodel_runtime::TxDialect, options: &litedbmodel_runtime::TransactionOptions, make_in: impl Fn() -> InNRNestedUpdate) -> Result<bool, litedbmodel_runtime::SqlFailure> {
    litedbmodel_runtime::run_transaction_on(src, connection, dialect, options, move |ctx| {
        run_native_raw_struct_NestedUpdate(&TxRt { ctx }, make_in()).map_err(|e| litedbmodel_runtime::SqlFailure { kind: e.code, policy: "fail".to_string(), sqlite_code: None, message: e.message })
    })
}
