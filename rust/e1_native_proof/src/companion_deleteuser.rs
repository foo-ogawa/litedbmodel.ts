...
use super::generated_deleteuser::*;
...
impl<'a> HandlerNRDeleteUser for Rt<'a> {
    ...
    fn node_n0(&self, ports: &PortsNRDeleteUserN0, _bound: Option<String>) -> Result<Wire, BehaviorError> {
        litedbmodel_runtime::exec(&litedbmodel_runtime::for_driver(self.driver), &ports.f_sql, &[litedbmodel_runtime::wp(&ports.f_p0)], litedbmodel_runtime::ExecMode::Summary).map_err(cvt)
    }
}

/// The litedbmodel-consumer entry: build the runtime-backed handler for `DeleteUser`. The consumer
/// calls `run_native_raw_struct_DeleteUser(&handler(driver), in_)` — supplying NO node_* itself.
