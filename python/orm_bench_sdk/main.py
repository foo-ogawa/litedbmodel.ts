...
# ── the canonical fixture, mirrored from the native twin (orm_bench.main). It is shared TEST DATA, not
#    covered code — each isolated SDK cell carries its own copy (exactly as the rust SDK carries its own
#    generated_setup), so this cell imports NOTHING from litedbmodel_runtime or the generated module. ──
SCHEMA: List[str] = [
