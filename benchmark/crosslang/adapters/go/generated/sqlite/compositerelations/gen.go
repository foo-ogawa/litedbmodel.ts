...
func RunNativeRawStruct_ByTenant(handlers Handler_ByTenant, in In_ByTenant) (T3, error) {
	...
	for _, oel_rel_comments := range over_rel_comments {
		ep_rel_comments := PortsNR_ByTenant_rel_comments{Sql: "SELECT tenant_id, comment_id, post_id, body FROM benchmark_tenant_comments WHERE EXISTS (SELECT 1 FROM json_each(?) je WHERE json_extract(je.value, '$[0]') = benchmark_tenant_comments.tenant_id AND json_extract(je.value, '$[1]') = benchmark_tenant_comments.post_id) ORDER BY comment_id ASC", K0: oel_rel_comments}
