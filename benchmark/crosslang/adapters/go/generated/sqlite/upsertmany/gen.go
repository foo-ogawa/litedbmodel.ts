...
func RunNativeRawStruct_UpsertMany(handlers Handler_UpsertMany, in In_UpsertMany) ([]T0, error) {
	...
	_ = produced_n0
	// ── op 'n0' (Insert) ──
	ports_n0 := PortsNR_UpsertMany_n0{Sql: "INSERT INTO benchmark_users (email, name) SELECT json_extract(value, '$.email'), json_extract(value, '$.name') FROM json_each(?) WHERE true ON CONFLICT (email) DO UPDATE SET email = excluded.email, name = excluded.name", V0: in.Emails, V1: in.Names}
