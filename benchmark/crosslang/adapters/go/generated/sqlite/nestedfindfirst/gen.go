...
func RunNativeRawStruct_FindFirst(handlers Handler_FindFirst, in In_FindFirst) (T2, error) {
	...
	for _, oel_rel_posts := range over_rel_posts {
		ep_rel_posts := PortsNR_FindFirst_rel_posts{Sql: "SELECT id, title, author_id FROM benchmark_posts WHERE author_id IN (SELECT value FROM json_each(?)) ORDER BY id ASC", K0: oel_rel_posts.Id}
