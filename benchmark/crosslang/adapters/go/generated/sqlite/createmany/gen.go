...
func RunNativeRawStruct_CreateMany(handlers Handler_CreateMany, in In_CreateMany) ([]T0, error) {
	...
	_ = produced_n0
	// ── op 'n0' (Insert) ──
	ports_n0 := PortsNR_CreateMany_n0{Sql: "INSERT INTO benchmark_users (email, name) SELECT json_extract(value, '$.email'), json_extract(value, '$.name') FROM json_each(?)", V0: in.Emails, V1: in.Names}
