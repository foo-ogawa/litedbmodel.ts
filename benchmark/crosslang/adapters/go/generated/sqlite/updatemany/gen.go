...
// PortsNR_UpdateMany_n0 — NATIVE ports for node 'n0' (Update). Typed fields per the
...
func RunNativeRawStruct_UpdateMany(handlers Handler_UpdateMany, in In_UpdateMany) ([]T0, error) {
	_ = in
	var t_n0 []T0
	produced_n0 := false
	_ = t_n0
	_ = produced_n0
	// ── op 'n0' (Update) ──
	ports_n0 := PortsNR_UpdateMany_n0{Sql: "UPDATE benchmark_users SET name = (SELECT json_extract(je.value, '$.name') FROM json_each(?) je WHERE json_extract(je.value, '$.id') = benchmark_users.id LIMIT 1) WHERE id IN (SELECT json_extract(value, '$.id') FROM json_each(?))", V0: in.Ids, V1: in.Names}
	wire_n0, wire_n0Err := handlers.Node_UpdateMany_n0(ports_n0, nil)
	if wire_n0Err != nil {
...
var ComponentNamesNativeRaw = []string{"UpdateMany"}
