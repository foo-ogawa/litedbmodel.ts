...
func RunNativeRawStruct_FindAll(handlers Handler_FindAll, in In_FindAll) (T3, error) {
	...
	for _, oel_rel_comments := range over_rel_comments {
		ep_rel_comments := PortsNR_FindAll_rel_comments{Sql: "SELECT id, body, post_id FROM benchmark_comments WHERE post_id IN (SELECT value FROM json_each(?)) ORDER BY id ASC", K0: oel_rel_comments}
