...
func RunNativeRawStruct_FindUnique(handlers Handler_FindUnique, in In_FindUnique) (T2, error) {
	...
	for _, oel_rel_posts := range over_rel_posts {
		ep_rel_posts := PortsNR_FindUnique_rel_posts{Sql: "SELECT id, title, author_id FROM benchmark_posts WHERE author_id IN (SELECT value FROM json_each(?)) ORDER BY id ASC", K0: oel_rel_posts.Id}
