...
// PortsNR_FilterPaginateSort_n0 — NATIVE ports for node 'n0' (Select). Typed fields per the
...
func RunNativeRawStruct_FilterPaginateSort(handlers Handler_FilterPaginateSort, in In_FilterPaginateSort) ([]T0, error) {
	_ = in
	var t_n0 []T0
	produced_n0 := false
	_ = t_n0
	_ = produced_n0
	// ── op 'n0' (Select) ──
	ports_n0 := PortsNR_FilterPaginateSort_n0{Sql: "SELECT id, title, content, published, author_id, created_at FROM benchmark_posts WHERE published = ? ORDER BY created_at DESC LIMIT ? OFFSET ?", P0: in.Published, P1: int64(20), P2: int64(10)}
	wire_n0, wire_n0Err := handlers.Node_FilterPaginateSort_n0(ports_n0, nil)
	if wire_n0Err != nil {
		...
		for i0 := 0; i0 < p0.Got.Len(); i0++ {
			...
			if p1.Kind == probeGot {
				...
				if p2.Kind == probeGot {
					...
					if nErr2 != nil {
						return nil, deOverflow("T0", "id", "int", p2.ActualWireType, p2.Got)
					}
					rec1.Id = n2
				} else if p2.Kind == probeWrong {
					return nil, deTypeMismatch("T0", "id", "int", p2.ActualWireType, p2.Raw)
				} else if p2.Kind == probeNull {
					return nil, deTypeMismatch("T0", "id", "int", p2.ActualWireType, p2.Raw)
				} else {
					return nil, deMissingField("T0", "id", "int")
				}
				p3 := p1.Got.ProbeString("title")
				if p3.Kind == probeGot {
					rec1.Title = p3.Got
				} else if p3.Kind == probeWrong {
					return nil, deTypeMismatch("T0", "title", "string", p3.ActualWireType, p3.Raw)
				} else if p3.Kind == probeNull {
					return nil, deTypeMismatch("T0", "title", "string", p3.ActualWireType, p3.Raw)
				} else {
					return nil, deMissingField("T0", "title", "string")
				}
				p4 := p1.Got.ProbeString("content")
				if p4.Kind == probeGot {
					rec1.Content = p4.Got
				} else if p4.Kind == probeWrong {
					return nil, deTypeMismatch("T0", "content", "string", p4.ActualWireType, p4.Raw)
				} else if p4.Kind == probeNull {
					return nil, deTypeMismatch("T0", "content", "string", p4.ActualWireType, p4.Raw)
				} else {
					return nil, deMissingField("T0", "content", "string")
				}
				p5 := p1.Got.ProbeNumber("published")
				if p5.Kind == probeGot {
					n5, nErr5 := strconv.ParseInt(p5.Got, 10, 64)
					if nErr5 != nil {
						return nil, deOverflow("T0", "published", "int", p5.ActualWireType, p5.Got)
					}
					rec1.Published = n5
				} else if p5.Kind == probeWrong {
					return nil, deTypeMismatch("T0", "published", "int", p5.ActualWireType, p5.Raw)
				} else if p5.Kind == probeNull {
					return nil, deTypeMismatch("T0", "published", "int", p5.ActualWireType, p5.Raw)
				} else {
					return nil, deMissingField("T0", "published", "int")
				}
				p6 := p1.Got.ProbeNumber("author_id")
				if p6.Kind == probeGot {
					n6, nErr6 := strconv.ParseInt(p6.Got, 10, 64)
					if nErr6 != nil {
						return nil, deOverflow("T0", "author_id", "int", p6.ActualWireType, p6.Got)
					}
					rec1.Author_id = n6
				} else if p6.Kind == probeWrong {
					return nil, deTypeMismatch("T0", "author_id", "int", p6.ActualWireType, p6.Raw)
				} else if p6.Kind == probeNull {
					return nil, deTypeMismatch("T0", "author_id", "int", p6.ActualWireType, p6.Raw)
				} else {
					return nil, deMissingField("T0", "author_id", "int")
				}
				p7 := p1.Got.ProbeString("created_at")
				if p7.Kind == probeGot {
					rec1.Created_at = p7.Got
				} else if p7.Kind == probeWrong {
					return nil, deTypeMismatch("T0", "created_at", "string", p7.ActualWireType, p7.Raw)
				} else if p7.Kind == probeNull {
					return nil, deTypeMismatch("T0", "created_at", "string", p7.ActualWireType, p7.Raw)
				} else {
					return nil, deMissingField("T0", "created_at", "string")
				}
				el0 = rec1
			} else if p1.Kind == probeWrong {
				return nil, deTypeMismatch("n0", "n0", "obj{id:int,title:string,content:string,published:int,author_id:int,created_at:string}", p1.ActualWireType, p1.Raw)
			} else if p1.Kind == probeNull {
				return nil, deTypeMismatch("n0", "n0", "obj{id:int,title:string,content:string,published:int,author_id:int,created_at:string}", p1.ActualWireType, p1.Raw)
			} else {
				return nil, deMissingField("n0", "n0", "obj{id:int,title:string,content:string,published:int,author_id:int,created_at:string}")
			}
			list0 = append(list0, el0)
		}
		t_n0 = list0
	} else if p0.Kind == probeWrong {
		return nil, deTypeMismatch("n0", "n0", "arr(obj{id:int,title:string,content:string,published:int,author_id:int,created_at:string})", p0.ActualWireType, p0.Raw)
	} else if p0.Kind == probeNull {
		return nil, deTypeMismatch("n0", "n0", "arr(obj{id:int,title:string,content:string,published:int,author_id:int,created_at:string})", p0.ActualWireType, p0.Raw)
	} else {
		return nil, deMissingField("n0", "n0", "arr(obj{id:int,title:string,content:string,published:int,author_id:int,created_at:string})")
	}
	produced_n0 = true
	return t_n0, nil
}
...
var ComponentNamesNativeRaw = []string{"FilterPaginateSort"}
