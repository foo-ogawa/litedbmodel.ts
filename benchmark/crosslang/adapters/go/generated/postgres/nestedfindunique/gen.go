...
func RunNativeRawStruct_FindUnique(handlers Handler_FindUnique, in In_FindUnique) (T2, error) {
	...
	// ── op 'n0' (Select) ──
	ports_n0 := PortsNR_FindUnique_n0{Sql: "SELECT id, email, name FROM benchmark_users WHERE email = $1 LIMIT $2", P0: in.Email, P1: int64(1)}
	...
	for _, oel_rel_posts := range over_rel_posts {
		ep_rel_posts := PortsNR_FindUnique_rel_posts{Sql: "SELECT id, title, author_id FROM benchmark_posts WHERE benchmark_posts.author_id = ANY(?::@@PG_ARRAY_CAST@@) ORDER BY id ASC", K0: oel_rel_posts.Id}
