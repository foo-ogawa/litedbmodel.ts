...
func RunNativeRawStruct_UpdateMany(handlers Handler_UpdateMany, in In_UpdateMany) ([]T0, error) {
	...
	_ = produced_n0
	// ── op 'n0' (Update) ──
	ports_n0 := PortsNR_UpdateMany_n0{Sql: "UPDATE benchmark_users AS t SET name = v.name FROM UNNEST($1::int[], $2::text[]) AS v(id, name) WHERE t.id = v.id", V0: in.Ids, V1: in.Names}
