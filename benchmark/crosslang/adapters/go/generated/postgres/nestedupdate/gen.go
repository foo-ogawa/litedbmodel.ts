...
// PortsNR_NestedUpdate_tx_body_0 — NATIVE ports for node 'tx_body_0' (Update). Typed fields per the
...
type PortsNR_NestedUpdate_tx_body_0 struct {
	Sql string // "sql"
	P0 string // "p0"
	P1 int64 // "p1"
}

// PortsNR_NestedUpdate_tx_body_1 — NATIVE ports for node 'tx_body_1' (Update). Typed fields per the
// static port type; built by direct field assignment (no map alloc, no per-port boxing). The consumer
// reads the typed fields DIRECTLY (no PortReader / boxed-Value accessor) — the covered read is native.
type PortsNR_NestedUpdate_tx_body_1 struct {
	Sql string // "sql"
	P0 string // "p0"
	P1 int64 // "p1"
}
...
func RunNativeRawStruct_NestedUpdate(handlers Handler_NestedUpdate, in In_NestedUpdate) (T1, error) {
	_ = in
	var t_tx_body_0 T0
	...
	// ── op 'tx_body_0' (Update) ──
	ports_tx_body_0 := PortsNR_NestedUpdate_tx_body_0{Sql: "UPDATE benchmark_users SET name = $1 WHERE id = $2", P0: in.Name, P1: in.User_id}
	wire_tx_body_0, wire_tx_body_0Err := handlers.Node_NestedUpdate_tx_body_0(ports_tx_body_0, nil)
	if wire_tx_body_0Err != nil {
		return T1{}, opFailed("tx_body_0", "fail", wire_tx_body_0Err)
	}
	p0 := wire_tx_body_0.AsRow()
	if p0.Kind == probeGot {
		var rec0 T0
		p1 := p0.Got.ProbeNumber("changes")
		if p1.Kind == probeGot {
			n1, nErr1 := strconv.ParseInt(p1.Got, 10, 64)
			if nErr1 != nil {
				return T1{}, deOverflow("T0", "changes", "int", p1.ActualWireType, p1.Got)
			}
			rec0.Changes = n1
		} else if p1.Kind == probeWrong {
			return T1{}, deTypeMismatch("T0", "changes", "int", p1.ActualWireType, p1.Raw)
		} else if p1.Kind == probeNull {
			return T1{}, deTypeMismatch("T0", "changes", "int", p1.ActualWireType, p1.Raw)
		} else {
			return T1{}, deMissingField("T0", "changes", "int")
		}
		p2 := p0.Got.ProbeNumber("lastInsertRowid")
		if p2.Kind == probeGot {
			n2, nErr2 := strconv.ParseInt(p2.Got, 10, 64)
			if nErr2 != nil {
				return T1{}, deOverflow("T0", "lastInsertRowid", "int", p2.ActualWireType, p2.Got)
			}
			rec0.LastInsertRowid = n2
		} else if p2.Kind == probeWrong {
			return T1{}, deTypeMismatch("T0", "lastInsertRowid", "int", p2.ActualWireType, p2.Raw)
		} else if p2.Kind == probeNull {
			return T1{}, deTypeMismatch("T0", "lastInsertRowid", "int", p2.ActualWireType, p2.Raw)
		} else {
			return T1{}, deMissingField("T0", "lastInsertRowid", "int")
		}
		t_tx_body_0 = rec0
	} else if p0.Kind == probeWrong {
		return T1{}, deTypeMismatch("tx_body_0", "tx_body_0", "obj{changes:int,lastInsertRowid:int}", p0.ActualWireType, p0.Raw)
	} else if p0.Kind == probeNull {
		return T1{}, deTypeMismatch("tx_body_0", "tx_body_0", "obj{changes:int,lastInsertRowid:int}", p0.ActualWireType, p0.Raw)
	} else {
		return T1{}, deMissingField("tx_body_0", "tx_body_0", "obj{changes:int,lastInsertRowid:int}")
	}
	produced_tx_body_0 = true
	// ── op 'tx_body_1' (Update) ──
	ports_tx_body_1 := PortsNR_NestedUpdate_tx_body_1{Sql: "UPDATE benchmark_posts SET title = $1 WHERE author_id = $2", P0: in.Title, P1: in.User_id}
	wire_tx_body_1, wire_tx_body_1Err := handlers.Node_NestedUpdate_tx_body_1(ports_tx_body_1, nil)
	if wire_tx_body_1Err != nil {
		return T1{}, opFailed("tx_body_1", "fail", wire_tx_body_1Err)
	}
	p3 := wire_tx_body_1.AsRow()
	if p3.Kind == probeGot {
		var rec3 T0
		p4 := p3.Got.ProbeNumber("changes")
		if p4.Kind == probeGot {
			n4, nErr4 := strconv.ParseInt(p4.Got, 10, 64)
			if nErr4 != nil {
				return T1{}, deOverflow("T0", "changes", "int", p4.ActualWireType, p4.Got)
			}
			rec3.Changes = n4
		} else if p4.Kind == probeWrong {
			return T1{}, deTypeMismatch("T0", "changes", "int", p4.ActualWireType, p4.Raw)
		} else if p4.Kind == probeNull {
			return T1{}, deTypeMismatch("T0", "changes", "int", p4.ActualWireType, p4.Raw)
		} else {
			return T1{}, deMissingField("T0", "changes", "int")
		}
		p5 := p3.Got.ProbeNumber("lastInsertRowid")
		if p5.Kind == probeGot {
			n5, nErr5 := strconv.ParseInt(p5.Got, 10, 64)
			if nErr5 != nil {
				return T1{}, deOverflow("T0", "lastInsertRowid", "int", p5.ActualWireType, p5.Got)
			}
			rec3.LastInsertRowid = n5
		} else if p5.Kind == probeWrong {
			return T1{}, deTypeMismatch("T0", "lastInsertRowid", "int", p5.ActualWireType, p5.Raw)
		...
		return T1{}, deTypeMismatch("tx_body_1", "tx_body_1", "obj{changes:int,lastInsertRowid:int}", p3.ActualWireType, p3.Raw)
	} else if p3.Kind == probeNull {
		return T1{}, deTypeMismatch("tx_body_1", "tx_body_1", "obj{changes:int,lastInsertRowid:int}", p3.ActualWireType, p3.Raw)
	} else {
		return T1{}, deMissingField("tx_body_1", "tx_body_1", "obj{changes:int,lastInsertRowid:int}")
	}
	produced_tx_body_1 = true
	return T1{User: t_tx_body_0, Post: t_tx_body_1}, nil
}
...
var ComponentNamesNativeRaw = []string{"NestedUpdate"}
