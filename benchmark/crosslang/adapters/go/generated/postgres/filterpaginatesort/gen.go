...
func RunNativeRawStruct_FilterPaginateSort(handlers Handler_FilterPaginateSort, in In_FilterPaginateSort) ([]T0, error) {
	...
	_ = produced_n0
	// ── op 'n0' (Select) ──
	ports_n0 := PortsNR_FilterPaginateSort_n0{Sql: "SELECT id, title, content, published, author_id, created_at FROM benchmark_posts WHERE published = $1 ORDER BY created_at DESC LIMIT $2 OFFSET $3", P0: in.Published, P1: int64(20), P2: int64(10)}
	...
	if p0.Kind == probeGot {
		...
		for i0 := 0; i0 < p0.Got.Len(); i0++ {
			...
			if p1.Kind == probeGot {
				...
				p5 := p1.Got.ProbeBool("published")
				if p5.Kind == probeGot {
					rec1.Published = p5.Got
				} else if p5.Kind == probeWrong {
					return nil, deTypeMismatch("T0", "published", "bool", p5.ActualWireType, p5.Raw)
				} else if p5.Kind == probeNull {
					return nil, deTypeMismatch("T0", "published", "bool", p5.ActualWireType, p5.Raw)
				} else {
					return nil, deMissingField("T0", "published", "bool")
				}
				...
				return nil, deTypeMismatch("n0", "n0", "obj{id:int,title:string,content:string,published:bool,author_id:int,created_at:string}", p1.ActualWireType, p1.Raw)
			} else if p1.Kind == probeNull {
				return nil, deTypeMismatch("n0", "n0", "obj{id:int,title:string,content:string,published:bool,author_id:int,created_at:string}", p1.ActualWireType, p1.Raw)
			} else {
				return nil, deMissingField("n0", "n0", "obj{id:int,title:string,content:string,published:bool,author_id:int,created_at:string}")
			}
			list0 = append(list0, el0)
		}
		t_n0 = list0
	} else if p0.Kind == probeWrong {
		return nil, deTypeMismatch("n0", "n0", "arr(obj{id:int,title:string,content:string,published:bool,author_id:int,created_at:string})", p0.ActualWireType, p0.Raw)
	} else if p0.Kind == probeNull {
		return nil, deTypeMismatch("n0", "n0", "arr(obj{id:int,title:string,content:string,published:bool,author_id:int,created_at:string})", p0.ActualWireType, p0.Raw)
	} else {
		return nil, deMissingField("n0", "n0", "arr(obj{id:int,title:string,content:string,published:bool,author_id:int,created_at:string})")
	}
	produced_n0 = true
	return t_n0, nil
}
