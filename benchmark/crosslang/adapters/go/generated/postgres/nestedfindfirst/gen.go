...
func RunNativeRawStruct_FindFirst(handlers Handler_FindFirst, in In_FindFirst) (T2, error) {
	...
	// ── op 'n0' (Select) ──
	ports_n0 := PortsNR_FindFirst_n0{Sql: "SELECT id, email, name FROM benchmark_users WHERE name LIKE $1 LIMIT $2", P0: in.Name, P1: int64(1)}
	...
	for _, oel_rel_posts := range over_rel_posts {
		ep_rel_posts := PortsNR_FindFirst_rel_posts{Sql: "SELECT id, title, author_id FROM benchmark_posts WHERE benchmark_posts.author_id = ANY(?::@@PG_ARRAY_CAST@@) ORDER BY id ASC", K0: oel_rel_posts.Id}
