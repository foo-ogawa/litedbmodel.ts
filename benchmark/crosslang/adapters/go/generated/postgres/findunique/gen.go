...
func RunNativeRawStruct_FindUnique(handlers Handler_FindUnique, in In_FindUnique) ([]T0, error) {
	_ = in
	var t_n0 []T0
	produced_n0 := false
	_ = t_n0
		...
		for i0 := 0; i0 < p0.Got.Len(); i0++ {
			...
			if p1.Kind == probeGot {
				...
				if p3.Kind == probeGot {
					...
					return nil, deTypeMismatch("T0", "email", "string", p3.ActualWireType, p3.Raw)
				} else if p3.Kind == probeNull {
					return nil, deTypeMismatch("T0", "email", "string", p3.ActualWireType, p3.Raw)
				} else {
					return nil, deMissingField("T0", "email", "string")
				}
				p4 := p1.Got.ProbeString("name")
				if p4.Kind == probeGot {
					rec1.Name = p4.Got
				} else if p4.Kind == probeWrong {
					return nil, deTypeMismatch("T0", "name", "string", p4.ActualWireType, p4.Raw)
				} else if p4.Kind == probeNull {
					return nil, deTypeMismatch("T0", "name", "string", p4.ActualWireType, p4.Raw)
				} else {
					return nil, deMissingField("T0", "name", "string")
				}
				el0 = rec1
			} else if p1.Kind == probeWrong {
				return nil, deTypeMismatch("n0", "n0", "obj{id:int,email:string,name:string}", p1.ActualWireType, p1.Raw)
			} else if p1.Kind == probeNull {
				return nil, deTypeMismatch("n0", "n0", "obj{id:int,email:string,name:string}", p1.ActualWireType, p1.Raw)
			} else {
				return nil, deMissingField("n0", "n0", "obj{id:int,email:string,name:string}")
			}
			list0 = append(list0, el0)
		}
		t_n0 = list0
	} else if p0.Kind == probeWrong {
		return nil, deTypeMismatch("n0", "n0", "arr(obj{id:int,email:string,name:string})", p0.ActualWireType, p0.Raw)
	} else if p0.Kind == probeNull {
		return nil, deTypeMismatch("n0", "n0", "arr(obj{id:int,email:string,name:string})", p0.ActualWireType, p0.Raw)
	} else {
		return nil, deMissingField("n0", "n0", "arr(obj{id:int,email:string,name:string})")
	}
	produced_n0 = true
	return t_n0, nil
}
