...
func RunNativeRawStruct_UpsertMany(handlers Handler_UpsertMany, in In_UpsertMany) ([]T0, error) {
	...
	_ = produced_n0
	// ── op 'n0' (Insert) ──
	ports_n0 := PortsNR_UpsertMany_n0{Sql: "INSERT INTO benchmark_users (email, name) SELECT v.email, v.name FROM UNNEST($1::text[], $2::text[]) AS v(email, name) ON CONFLICT (email) DO UPDATE SET email = EXCLUDED.email, name = EXCLUDED.name", V0: in.Emails, V1: in.Names}
