...
// Row_Upsert_n0 — CONCRETE handler-result row (typed fields = the node's outType; + error signal).
type Row_Upsert_n0 struct {
	IsError bool
	Err     string
	Val []T0
}
...
// Handler_Upsert — the CONCRETE per-component handler seam: one typed method
...
type Handler_Upsert interface {
	Node_Upsert_n0(ports PortsNR_Upsert_n0, bound *string) (Row_Upsert_n0, bool)
}
...
func RunNativeRawStruct_Upsert[H Handler_Upsert](handlers H, in In_Upsert) ([]T0, error) {
	_ = in
	var t_n0 []T0
	produced_n0 := false
	_ = t_n0
	_ = produced_n0
	// ── op 'n0' (Insert) ──
	ports_n0 := PortsNR_Upsert_n0{Sql: "INSERT INTO benchmark_users (email, name) VALUES (?, ?) ON CONFLICT (email) DO UPDATE SET email = excluded.email, name = excluded.name RETURNING id, email, name", P0: in.Email, P1: in.Name}
	row_n0, row_n0Resolved := handlers.Node_Upsert_n0(ports_n0, nil)
	if !row_n0Resolved {
