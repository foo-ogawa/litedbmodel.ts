...
// Row_UpdateMany_n0 — CONCRETE handler-result row (typed fields = the node's outType; + error signal).
type Row_UpdateMany_n0 struct {
	IsError bool
	Err     string
	Val []T0
}
...
// Handler_UpdateMany — the CONCRETE per-component handler seam: one typed method
...
type Handler_UpdateMany interface {
	Node_UpdateMany_n0(ports PortsNR_UpdateMany_n0, bound *string) (Row_UpdateMany_n0, bool)
}
...
func RunNativeRawStruct_UpdateMany[H Handler_UpdateMany](handlers H, in In_UpdateMany) ([]T0, error) {
	_ = in
	var t_n0 []T0
	produced_n0 := false
	_ = t_n0
	_ = produced_n0
	// ── op 'n0' (Update) ──
	ports_n0 := PortsNR_UpdateMany_n0{Sql: "UPDATE benchmark_users SET name = (SELECT json_extract(je.value, '$.name') FROM json_each(?) je WHERE json_extract(je.value, '$.id') = benchmark_users.id LIMIT 1) WHERE id IN (SELECT json_extract(value, '$.id') FROM json_each(?)) RETURNING id, email, name", V0: in.Ids, V1: in.Names}
	row_n0, row_n0Resolved := handlers.Node_UpdateMany_n0(ports_n0, nil)
	if !row_n0Resolved {
		return nil, &BehaviorError{Code: "UNKNOWN_COMPONENT", Message: "component 'Update' has no handler (fail-closed)"}
	}
	if row_n0.IsError {
