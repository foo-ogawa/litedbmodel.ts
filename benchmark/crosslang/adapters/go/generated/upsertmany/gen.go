...
// Row_UpsertMany_n0 — CONCRETE handler-result row (typed fields = the node's outType; + error signal).
type Row_UpsertMany_n0 struct {
	IsError bool
	Err     string
	Val []T0
}
...
// Handler_UpsertMany — the CONCRETE per-component handler seam: one typed method
...
type Handler_UpsertMany interface {
	Node_UpsertMany_n0(ports PortsNR_UpsertMany_n0, bound *string) (Row_UpsertMany_n0, bool)
}
...
func RunNativeRawStruct_UpsertMany[H Handler_UpsertMany](handlers H, in In_UpsertMany) ([]T0, error) {
	_ = in
	var t_n0 []T0
	produced_n0 := false
	_ = t_n0
	_ = produced_n0
	// ── op 'n0' (Insert) ──
	ports_n0 := PortsNR_UpsertMany_n0{Sql: "INSERT INTO benchmark_users (email, name) SELECT json_extract(value, '$.email'), json_extract(value, '$.name') FROM json_each(?) WHERE true ON CONFLICT (email) DO UPDATE SET email = excluded.email, name = excluded.name RETURNING id, email, name", V0: in.Emails, V1: in.Names}
	row_n0, row_n0Resolved := handlers.Node_UpsertMany_n0(ports_n0, nil)
	if !row_n0Resolved {
