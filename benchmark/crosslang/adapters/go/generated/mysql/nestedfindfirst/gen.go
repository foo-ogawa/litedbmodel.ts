...
// PortsNR_FindFirst_n0 — NATIVE ports for node 'n0' (Select). Typed fields per the
...
// PortsNR_FindFirst_rel_posts — NATIVE ports for node 'rel_posts' (Select). Typed fields per the
// static port type; built by direct field assignment (no map alloc, no per-port boxing). The consumer
// reads the typed fields DIRECTLY (no PortReader / boxed-Value accessor) — the covered read is native.
type PortsNR_FindFirst_rel_posts struct {
	Sql string // "sql"
	K0 int64 // "k0"
}

// PortsNR_FindFirst_rel_posts_batch — NATIVE batched ports for map 'rel_posts': Items is a native slice of the concrete
// per-element ports structs (PortsNR_FindFirst_rel_posts). The consumer reads Items[i].<Field> DIRECTLY — no boxed
// []Value, no PortReader accessor, no key-value object (bc#90 / runtime-free).
type PortsNR_FindFirst_rel_posts_batch struct {
	Items []PortsNR_FindFirst_rel_posts
}
...
func RunNativeRawStruct_FindFirst(handlers Handler_FindFirst, in In_FindFirst) (T2, error) {
	_ = in
	var t_n0 []T0
	produced_n0 := false
	_ = t_n0
	...
	// ── op 'n0' (Select) ──
	ports_n0 := PortsNR_FindFirst_n0{Sql: "SELECT id, email, name FROM benchmark_users WHERE name LIKE ? LIMIT ?", P0: in.Name, P1: int64(1)}
	wire_n0, wire_n0Err := handlers.Node_FindFirst_n0(ports_n0, nil)
	if wire_n0Err != nil {
	...
	items_rel_posts := make([]PortsNR_FindFirst_rel_posts, 0, len(over_rel_posts))
	for _, oel_rel_posts := range over_rel_posts {
		ep_rel_posts := PortsNR_FindFirst_rel_posts{Sql: "SELECT id, title, author_id FROM benchmark_posts WHERE author_id IN (SELECT JSON_UNQUOTE(v) FROM JSON_TABLE(?, '$[*]' COLUMNS(v JSON PATH '$')) jt) ORDER BY id ASC", K0: oel_rel_posts.Id}
		items_rel_posts = append(items_rel_posts, ep_rel_posts)
	}
	if len(items_rel_posts) > 0 {
		bp_rel_posts := PortsNR_FindFirst_rel_posts_batch{Items: items_rel_posts}
		mo_rel_posts, mo_rel_postsErr := handlers.Node_FindFirst_rel_posts(bp_rel_posts, nil)
...
var ComponentNamesNativeRaw = []string{"FindFirst"}
