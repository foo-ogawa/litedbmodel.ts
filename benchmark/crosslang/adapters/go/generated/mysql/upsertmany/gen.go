...
// PortsNR_UpsertMany_n0 — NATIVE ports for node 'n0' (Insert). Typed fields per the
...
func RunNativeRawStruct_UpsertMany(handlers Handler_UpsertMany, in In_UpsertMany) ([]T0, error) {
	_ = in
	var t_n0 []T0
	produced_n0 := false
	_ = t_n0
	_ = produced_n0
	// ── op 'n0' (Insert) ──
	ports_n0 := PortsNR_UpsertMany_n0{Sql: "INSERT INTO benchmark_users (email, name) SELECT JSON_UNQUOTE(jt.email), JSON_UNQUOTE(jt.name) FROM JSON_TABLE(?, '$[*]' COLUMNS(email JSON PATH '$.email', name JSON PATH '$.name')) jt ON DUPLICATE KEY UPDATE email = VALUES(email), name = VALUES(name)", V0: in.Emails, V1: in.Names}
	wire_n0, wire_n0Err := handlers.Node_UpsertMany_n0(ports_n0, nil)
	if wire_n0Err != nil {
		...
		for i0 := 0; i0 < p0.Got.Len(); i0++ {
			...
			if p1.Kind == probeGot {
				...
				if p2.Kind == probeGot {
					...
					if nErr2 != nil {
						return nil, deOverflow("T0", "changes", "int", p2.ActualWireType, p2.Got)
					}
					rec1.Changes = n2
				} else if p2.Kind == probeWrong {
					return nil, deTypeMismatch("T0", "changes", "int", p2.ActualWireType, p2.Raw)
				} else if p2.Kind == probeNull {
					return nil, deTypeMismatch("T0", "changes", "int", p2.ActualWireType, p2.Raw)
				} else {
					return nil, deMissingField("T0", "changes", "int")
				}
				p3 := p1.Got.ProbeNumber("lastInsertRowid")
				if p3.Kind == probeGot {
					n3, nErr3 := strconv.ParseInt(p3.Got, 10, 64)
					if nErr3 != nil {
						return nil, deOverflow("T0", "lastInsertRowid", "int", p3.ActualWireType, p3.Got)
					}
					rec1.LastInsertRowid = n3
				} else if p3.Kind == probeWrong {
					return nil, deTypeMismatch("T0", "lastInsertRowid", "int", p3.ActualWireType, p3.Raw)
				} else if p3.Kind == probeNull {
					return nil, deTypeMismatch("T0", "lastInsertRowid", "int", p3.ActualWireType, p3.Raw)
				} else {
					return nil, deMissingField("T0", "lastInsertRowid", "int")
				}
				el0 = rec1
...
var ComponentNamesNativeRaw = []string{"UpsertMany"}
