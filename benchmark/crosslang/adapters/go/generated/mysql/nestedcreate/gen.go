...
// PortsNR_NestedCreate_tx_body_0 — NATIVE ports for node 'tx_body_0' (Insert). Typed fields per the
...
type PortsNR_NestedCreate_tx_body_0 struct {
	Sql string // "sql"
	P0 string // "p0"
	P1 string // "p1"
}

// PortsNR_NestedCreate_tx_body_1 — NATIVE ports for node 'tx_body_1' (Insert). Typed fields per the
// static port type; built by direct field assignment (no map alloc, no per-port boxing). The consumer
// reads the typed fields DIRECTLY (no PortReader / boxed-Value accessor) — the covered read is native.
type PortsNR_NestedCreate_tx_body_1 struct {
	Sql string // "sql"
	P0 int64 // "p0"
	P1 string // "p1"
}
...
func RunNativeRawStruct_NestedCreate(handlers Handler_NestedCreate, in In_NestedCreate) (T2, error) {
	_ = in
	var t_tx_body_0 T0
	...
	// ── op 'tx_body_0' (Insert) ──
	ports_tx_body_0 := PortsNR_NestedCreate_tx_body_0{Sql: "INSERT INTO benchmark_users (email, name) VALUES (?, ?) /*scp-reselect: SELECT id FROM benchmark_users WHERE id >= ? AND id < ? ORDER BY id ::binds:: L,H*/", P0: in.Email, P1: in.Name}
	wire_tx_body_0, wire_tx_body_0Err := handlers.Node_NestedCreate_tx_body_0(ports_tx_body_0, nil)
	...
	if produced_tx_body_0 {
		ports_tx_body_1 := PortsNR_NestedCreate_tx_body_1{Sql: "INSERT INTO benchmark_posts (author_id, title) VALUES (?, ?)", P0: t_tx_body_0.Id, P1: in.Title}
		wire_tx_body_1, wire_tx_body_1Err := handlers.Node_NestedCreate_tx_body_1(ports_tx_body_1, nil)
...
var ComponentNamesNativeRaw = []string{"NestedCreate"}
