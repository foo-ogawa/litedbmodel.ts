...
// PortsNR_CreateMany_n0 — NATIVE ports for node 'n0' (Insert). Typed fields per the
...
func RunNativeRawStruct_CreateMany(handlers Handler_CreateMany, in In_CreateMany) ([]T0, error) {
	_ = in
	var t_n0 []T0
	produced_n0 := false
	_ = t_n0
	_ = produced_n0
	// ── op 'n0' (Insert) ──
	ports_n0 := PortsNR_CreateMany_n0{Sql: "INSERT INTO benchmark_users (email, name) SELECT JSON_UNQUOTE(jt.email), JSON_UNQUOTE(jt.name) FROM JSON_TABLE(?, '$[*]' COLUMNS(email JSON PATH '$.email', name JSON PATH '$.name')) jt", V0: in.Emails, V1: in.Names}
	wire_n0, wire_n0Err := handlers.Node_CreateMany_n0(ports_n0, nil)
	if wire_n0Err != nil {
...
var ComponentNamesNativeRaw = []string{"CreateMany"}
