...
// PortsNR_FindUnique_n0 — NATIVE ports for node 'n0' (Select). Typed fields per the
...
// PortsNR_FindUnique_rel_posts — NATIVE ports for node 'rel_posts' (Select). Typed fields per the
// static port type; built by direct field assignment (no map alloc, no per-port boxing). The consumer
// reads the typed fields DIRECTLY (no PortReader / boxed-Value accessor) — the covered read is native.
type PortsNR_FindUnique_rel_posts struct {
	Sql string // "sql"
	K0 int64 // "k0"
}

// PortsNR_FindUnique_rel_posts_batch — NATIVE batched ports for map 'rel_posts': Items is a native slice of the concrete
// per-element ports structs (PortsNR_FindUnique_rel_posts). The consumer reads Items[i].<Field> DIRECTLY — no boxed
// []Value, no PortReader accessor, no key-value object (bc#90 / runtime-free).
type PortsNR_FindUnique_rel_posts_batch struct {
	Items []PortsNR_FindUnique_rel_posts
}
...
func RunNativeRawStruct_FindUnique(handlers Handler_FindUnique, in In_FindUnique) (T2, error) {
	_ = in
	var t_n0 []T0
	produced_n0 := false
	_ = t_n0
	...
	// ── op 'n0' (Select) ──
	ports_n0 := PortsNR_FindUnique_n0{Sql: "SELECT id, email, name FROM benchmark_users WHERE email = ? LIMIT ?", P0: in.Email, P1: int64(1)}
	wire_n0, wire_n0Err := handlers.Node_FindUnique_n0(ports_n0, nil)
	if wire_n0Err != nil {
		...
		for i0 := 0; i0 < p0.Got.Len(); i0++ {
			...
			if p1.Kind == probeGot {
				...
				if p3.Kind == probeGot {
					...
					return T2{}, deTypeMismatch("T0", "email", "string", p3.ActualWireType, p3.Raw)
				} else if p3.Kind == probeNull {
					return T2{}, deTypeMismatch("T0", "email", "string", p3.ActualWireType, p3.Raw)
				} else {
					return T2{}, deMissingField("T0", "email", "string")
				}
				...
				return T2{}, deTypeMismatch("n0", "n0", "obj{id:int,email:string,name:string}", p1.ActualWireType, p1.Raw)
			} else if p1.Kind == probeNull {
				return T2{}, deTypeMismatch("n0", "n0", "obj{id:int,email:string,name:string}", p1.ActualWireType, p1.Raw)
			} else {
				return T2{}, deMissingField("n0", "n0", "obj{id:int,email:string,name:string}")
			}
			list0 = append(list0, el0)
		}
		t_n0 = list0
	} else if p0.Kind == probeWrong {
		return T2{}, deTypeMismatch("n0", "n0", "arr(obj{id:int,email:string,name:string})", p0.ActualWireType, p0.Raw)
	} else if p0.Kind == probeNull {
		return T2{}, deTypeMismatch("n0", "n0", "arr(obj{id:int,email:string,name:string})", p0.ActualWireType, p0.Raw)
	} else {
		return T2{}, deMissingField("n0", "n0", "arr(obj{id:int,email:string,name:string})")
	}
	produced_n0 = true
	// ── map 'rel_posts' (Select, batched, into:null, relationKind:connection) ──
	over_rel_posts := t_n0
	t_rel_posts = make([][]T1, 0, len(over_rel_posts))
	items_rel_posts := make([]PortsNR_FindUnique_rel_posts, 0, len(over_rel_posts))
	for _, oel_rel_posts := range over_rel_posts {
		ep_rel_posts := PortsNR_FindUnique_rel_posts{Sql: "SELECT id, title, author_id FROM benchmark_posts WHERE author_id IN (SELECT JSON_UNQUOTE(v) FROM JSON_TABLE(?, '$[*]' COLUMNS(v JSON PATH '$')) jt) ORDER BY id ASC", K0: oel_rel_posts.Id}
		items_rel_posts = append(items_rel_posts, ep_rel_posts)
	}
	if len(items_rel_posts) > 0 {
		bp_rel_posts := PortsNR_FindUnique_rel_posts_batch{Items: items_rel_posts}
		mo_rel_posts, mo_rel_postsErr := handlers.Node_FindUnique_rel_posts(bp_rel_posts, nil)
				...
				for i0 := 0; i0 < p0.Got.Len(); i0++ {
					...
					if p1.Kind == probeGot {
						...
						if p3.Kind == probeGot {
							...
							return T2{}, deTypeMismatch("T1", "title", "string", p3.ActualWireType, p3.Raw)
						} else if p3.Kind == probeNull {
							return T2{}, deTypeMismatch("T1", "title", "string", p3.ActualWireType, p3.Raw)
						} else {
						...
						if p4.Kind == probeGot {
							...
							if nErr4 != nil {
								return T2{}, deOverflow("T1", "author_id", "int", p4.ActualWireType, p4.Got)
							}
							rec1.Author_id = n4
						} else if p4.Kind == probeWrong {
							return T2{}, deTypeMismatch("T1", "author_id", "int", p4.ActualWireType, p4.Raw)
						} else if p4.Kind == probeNull {
							return T2{}, deTypeMismatch("T1", "author_id", "int", p4.ActualWireType, p4.Raw)
						} else {
							return T2{}, deMissingField("T1", "author_id", "int")
						}
						el0 = rec1
					} else if p1.Kind == probeWrong {
						return T2{}, deTypeMismatch("rel_posts", "rel_posts", "obj{id:int,title:string,author_id:int}", p1.ActualWireType, p1.Raw)
					} else if p1.Kind == probeNull {
						return T2{}, deTypeMismatch("rel_posts", "rel_posts", "obj{id:int,title:string,author_id:int}", p1.ActualWireType, p1.Raw)
					} else {
						return T2{}, deMissingField("rel_posts", "rel_posts", "obj{id:int,title:string,author_id:int}")
					}
					list0 = append(list0, el0)
				}
				el_rel_posts = list0
			} else if p0.Kind == probeWrong {
				return T2{}, deTypeMismatch("rel_posts", "rel_posts", "arr(obj{id:int,title:string,author_id:int})", p0.ActualWireType, p0.Raw)
			} else if p0.Kind == probeNull {
				return T2{}, deTypeMismatch("rel_posts", "rel_posts", "arr(obj{id:int,title:string,author_id:int})", p0.ActualWireType, p0.Raw)
			} else {
				return T2{}, deMissingField("rel_posts", "rel_posts", "arr(obj{id:int,title:string,author_id:int})")
			}
...
var ComponentNamesNativeRaw = []string{"FindUnique"}
