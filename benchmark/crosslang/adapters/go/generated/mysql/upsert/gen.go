...
// PortsNR_Upsert_n0 — NATIVE ports for node 'n0' (Insert). Typed fields per the
...
func RunNativeRawStruct_Upsert(handlers Handler_Upsert, in In_Upsert) ([]T0, error) {
	_ = in
	var t_n0 []T0
	produced_n0 := false
	_ = t_n0
	_ = produced_n0
	// ── op 'n0' (Insert) ──
	ports_n0 := PortsNR_Upsert_n0{Sql: "INSERT INTO benchmark_users (email, name) VALUES (?, ?) ON DUPLICATE KEY UPDATE email = VALUES(email), name = VALUES(name) /*scp-reselect: SELECT id FROM benchmark_users WHERE email = ? ORDER BY id ::binds:: p0*/", P0: in.Email, P1: in.Name}
	wire_n0, wire_n0Err := handlers.Node_Upsert_n0(ports_n0, nil)
	if wire_n0Err != nil {
...
var ComponentNamesNativeRaw = []string{"Upsert"}
