...
func RunNativeRawStruct_ByTenant(handlers Handler_ByTenant, in In_ByTenant) (T3, error) {
	...
	for _, oel_rel_comments := range over_rel_comments {
		ep_rel_comments := PortsNR_ByTenant_rel_comments{Sql: "SELECT tenant_id, comment_id, post_id, body FROM benchmark_tenant_comments WHERE (benchmark_tenant_comments.tenant_id, benchmark_tenant_comments.post_id) IN (SELECT JSON_UNQUOTE(c0), JSON_UNQUOTE(c1) FROM JSON_TABLE(?, '$[*]' COLUMNS(c0 JSON PATH '$[0]', c1 JSON PATH '$[1]')) jt) ORDER BY comment_id ASC", K0: oel_rel_comments}
