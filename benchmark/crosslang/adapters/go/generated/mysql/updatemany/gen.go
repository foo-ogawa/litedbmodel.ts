...
func RunNativeRawStruct_UpdateMany(handlers Handler_UpdateMany, in In_UpdateMany) ([]T0, error) {
	...
	_ = produced_n0
	// ── op 'n0' (Update) ──
	ports_n0 := PortsNR_UpdateMany_n0{Sql: "UPDATE benchmark_users AS u JOIN JSON_TABLE(?, '$[*]' COLUMNS(id JSON PATH '$.id', name JSON PATH '$.name')) AS v ON u.id = JSON_UNQUOTE(v.id) SET u.name = JSON_UNQUOTE(v.name)", V0: in.Ids, V1: in.Names}
