...
// Row_Update_n0 — CONCRETE handler-result row (typed fields = the node's outType; + error signal).
type Row_Update_n0 struct {
	IsError bool
	Err     string
	Val []T0
}
...
// Handler_Update — the CONCRETE per-component handler seam: one typed method
...
type Handler_Update interface {
	Node_Update_n0(ports PortsNR_Update_n0, bound *string) (Row_Update_n0, bool)
}
...
func RunNativeRawStruct_Update[H Handler_Update](handlers H, in In_Update) ([]T0, error) {
	_ = in
	var t_n0 []T0
	produced_n0 := false
	_ = t_n0
	_ = produced_n0
	// ── op 'n0' (Update) ──
	ports_n0 := PortsNR_Update_n0{Sql: "UPDATE benchmark_users SET name = ? WHERE id = ? RETURNING id, email, name", P0: in.Name, P1: in.Id}
	row_n0, row_n0Resolved := handlers.Node_Update_n0(ports_n0, nil)
	if !row_n0Resolved {
