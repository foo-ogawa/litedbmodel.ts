...
// Row_FilterPaginateSort_n0 — CONCRETE handler-result row (typed fields = the node's outType; + error signal).
type Row_FilterPaginateSort_n0 struct {
	IsError bool
	Err     string
	Val []T0
}
...
// Handler_FilterPaginateSort — the CONCRETE per-component handler seam: one typed method
...
type Handler_FilterPaginateSort interface {
	Node_FilterPaginateSort_n0(ports PortsNR_FilterPaginateSort_n0, bound *string) (Row_FilterPaginateSort_n0, bool)
}
...
func RunNativeRawStruct_FilterPaginateSort[H Handler_FilterPaginateSort](handlers H, in In_FilterPaginateSort) ([]T0, error) {
	_ = in
	var t_n0 []T0
	produced_n0 := false
	_ = t_n0
	...
	row_n0, row_n0Resolved := handlers.Node_FilterPaginateSort_n0(ports_n0, nil)
	if !row_n0Resolved {
