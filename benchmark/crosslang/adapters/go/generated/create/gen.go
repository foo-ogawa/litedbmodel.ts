...
// Row_Create_n0 — CONCRETE handler-result row (typed fields = the node's outType; + error signal).
type Row_Create_n0 struct {
	IsError bool
	Err     string
	Val []T0
}
...
// Handler_Create — the CONCRETE per-component handler seam: one typed method
...
type Handler_Create interface {
	Node_Create_n0(ports PortsNR_Create_n0, bound *string) (Row_Create_n0, bool)
}
...
func RunNativeRawStruct_Create[H Handler_Create](handlers H, in In_Create) ([]T0, error) {
	_ = in
	var t_n0 []T0
	produced_n0 := false
	_ = t_n0
	_ = produced_n0
	// ── op 'n0' (Insert) ──
	ports_n0 := PortsNR_Create_n0{Sql: "INSERT INTO benchmark_users (email, name) VALUES (?, ?) RETURNING id, email, name", P0: in.Email, P1: in.Name}
	row_n0, row_n0Resolved := handlers.Node_Create_n0(ports_n0, nil)
	if !row_n0Resolved {
