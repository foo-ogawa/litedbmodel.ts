// GENERATED by behavior-contracts (bc#90, go-typed-native — the RUNTIME-FREE covered read de-box) — DO NOT EDIT.
// Input: portable component-graph IR only. Every component here is a COVERED READ (sequential typed
// componentRef); it is emitted ONLY as the combined native ports IN + raw struct OUT + inline
// sequential runner below. There is NO generic Bind/dispatch/plan scaffolding AND NO bc-runtime import —
// a fully-covered module is FULLY NATIVE (zero bc-runtime reference; only the std-lib it uses).

package nestedfindfirst

import (
	"fmt"
)

// ExpectedSpecVersions are the spec versions baked at generation time (provenance only — the module
// is runtime-free, so there is no runtime SpecVersions to skew-check against, bc#90 / change #5).
var ExpectedSpecVersions = map[string]int64{"behavior": 5, "expression": 2, "plan": 1}

// COMBINED READ layer (bc#90 — the RUNTIME-FREE read de-box; STRUCT-ONLY, FULLY-NATIVE surface). A
// covered read is a strictly-sequential typed componentRef chain (point reads + relationKind:single
// children), emitted ONLY here: each node builds a native ports struct by direct field assignment (every
// port is a CONCRETE Go value — string / bool / []string projection / int64 limit — NO boxed Value, NO
// PortReader accessor); the CONCRETE per-component handler interface (Handler_<comp>) takes a typed
// produced-aware 'bound' pointer and returns a CONCRETE per-node row struct (Row_<comp>_<node>, typed
// fields = the node's outType) whose fields the runner reads DIRECTLY and copies into the node's outType
// struct. The covered plane carries NO bc-runtime reference at all — failures use the LOCAL
// *BehaviorError (same codes, byte-equal to run_behavior). Exec is INLINE sequential (no plan driver) so
// a relation child reads the parent's REAL struct result via direct field access and the child-present
// decision is made from the real parent value — relationSingle / connection converge with run_behavior
// (fixes #323/#74). The exposed API is the STRUCT-returning RunNativeRawStruct_<comp> (the consumer
// keeps the model native — no serialization on the read hot path). This module is FULLY NATIVE: a naive
// grep for boxing primitives OR for the bc-runtime package finds nothing, by design (bc#90/runtime-free).

// Local concrete failure type (runtime-free) — a covered runner returns *BehaviorError (satisfies
// error) instead of a bc-runtime failure, so the fully-covered module imports ZERO bc runtime.
// BehaviorError — the LOCAL concrete failure type for the covered read plane (runtime-free): a
// covered runner returns *BehaviorError (which satisfies error) instead of a bc-runtime failure, so
// the fully-covered module imports ZERO bc runtime. Codes match run_behavior verbatim (byte-equal).
type BehaviorError struct {
	Code    string
	Message string
}

func (e *BehaviorError) Error() string { return e.Code + ": " + e.Message }

// FailureCode exposes the stable failure code (byte-equal to run_behavior) WITHOUT a bc-runtime type:
// a caller that wants the code type-asserts the small local interface { FailureCode() string } rather
// than a bc failure type — keeping this module runtime-free while a harness can still read the code.
func (e *BehaviorError) FailureCode() string { return e.Code }

// Typed struct declarations (outType-derived; hash-dedup — shared type plan).
type T0 struct {
	Id    int64  // "id"
	Email string // "email"
	Name  string // "name"
}

type T1 struct {
	Id        int64  // "id"
	Title     string // "title"
	Author_id int64  // "author_id"
}

type T2 struct {
	Rows  []T0   // "rows"
	Posts [][]T1 // "posts"
}

// Per-component CONCRETE input structs (fields = inputPorts; the consumer builds them natively —
// no generic *Obj crosses the covered read boundary).
// In_FindFirst — the CONCRETE input for 'FindFirst' (fields = inputPorts; typed, consumer-built —
// NO generic *Obj, NO per-field boxing crosses the covered read boundary).
type In_FindFirst struct {
	Name string // "name"
}

// CONCRETE per-node handler-result row structs (typed fields = the node's outType; + error signal).
// Row_FindFirst_n0 — CONCRETE handler-result row (typed fields = the node's outType; + error signal).
type Row_FindFirst_n0 struct {
	IsError bool
	Err     string
	Val []T0
}

// RowElem_FindFirst_rel_posts — CONCRETE handler-result row (typed fields = the node's outType; + error signal).
type RowElem_FindFirst_rel_posts struct {
	IsError bool
	Err     string
	Val []T1
}

// Row_FindFirst_rel_posts — the BATCH row for map 'rel_posts': the aligned per-element rows.
type Row_FindFirst_rel_posts struct {
	IsError bool
	Err     string
	Rows    []RowElem_FindFirst_rel_posts
}

// Per-component CONCRETE handler interfaces (one typed Node_* method per covered node).
// Handler_FindFirst — the CONCRETE per-component handler seam: one typed method
// per covered node (native ports struct IN, concrete row struct OUT). No generic boxed-ports
// / boxed-value / dynamic accessor crosses the covered boundary — the consumer implements each
// Node_* with a decode of its own wire payload into the concrete row (see INTEGRATION.md §6).
type Handler_FindFirst interface {
	Node_FindFirst_n0(ports PortsNR_FindFirst_n0, bound *string) (Row_FindFirst_n0, bool)
	Node_FindFirst_rel_posts(ports PortsNR_FindFirst_rel_posts_batch, bound *string) (Row_FindFirst_rel_posts, bool)
}

// Native ports structs (one per componentRef node; typed per the static port type).
// PortsNR_FindFirst_n0 — NATIVE ports for node 'n0' (Select). Typed fields per the
// static port type; built by direct field assignment (no map alloc, no per-port boxing). The consumer
// reads the typed fields DIRECTLY (no PortReader / boxed-Value accessor) — the covered read is native.
type PortsNR_FindFirst_n0 struct {
	Sql string // "sql"
	P0 string // "p0"
	P1 int64 // "p1"
}

// PortsNR_FindFirst_rel_posts — NATIVE ports for node 'rel_posts' (Select). Typed fields per the
// static port type; built by direct field assignment (no map alloc, no per-port boxing). The consumer
// reads the typed fields DIRECTLY (no PortReader / boxed-Value accessor) — the covered read is native.
type PortsNR_FindFirst_rel_posts struct {
	Sql string // "sql"
	K0 int64 // "k0"
}

// PortsNR_FindFirst_rel_posts_batch — NATIVE batched ports for map 'rel_posts': Items is a native slice of the concrete
// per-element ports structs (PortsNR_FindFirst_rel_posts). The consumer reads Items[i].<Field> DIRECTLY — no boxed
// []Value, no PortReader accessor, no key-value object (bc#90 / runtime-free).
type PortsNR_FindFirst_rel_posts_batch struct {
	Items []PortsNR_FindFirst_rel_posts
}

// Combined read runners (STRUCT-returning — the fully de-plumbed path).
// RunNativeRawStruct_FindFirst — the STRUCT-RETURNING combined read (bc#77/#87): the fully
// de-plumbed path. Ports are a native struct (direct field assignment); the handler
// result is materialized straight into the node's outType struct. Node results are typed
// struct locals; a relation child reads the parent's REAL struct result via direct field
// access (child-present decision from the real parent value — relationSingle / connection
// converge). A strictly-sequential plan is a plain statement list (no plan driver); a
// real-concurrency stage (bc#87) is EXPLICIT static parallel orchestration — one goroutine
// per statically-known parallel op, bounded by the static plan.concurrency via a semaphore
// of known size, with a sync.WaitGroup; preflight + interpret are committed in ascending
// index order so the observed value / op multiset / failure precedence byte-match run_behavior
// (the goroutine spawn is the ONLY runtime element — the WHAT is baked). The output is a typed
// struct/value assembled by struct literal + field access — the consumer keeps it native.
func RunNativeRawStruct_FindFirst[H Handler_FindFirst](handlers H, in In_FindFirst) (T2, error) {
	_ = in
	var t_n0 []T0
	produced_n0 := false
	_ = t_n0
	_ = produced_n0
	var t_rel_posts [][]T1
	produced_rel_posts := false
	_ = t_rel_posts
	_ = produced_rel_posts
	// ── op 'n0' (Select) ──
	ports_n0 := PortsNR_FindFirst_n0{Sql: "SELECT id, email, name FROM benchmark_users WHERE name LIKE ? LIMIT ?", P0: in.Name, P1: int64(1)}
	row_n0, row_n0Resolved := handlers.Node_FindFirst_n0(ports_n0, nil)
	if !row_n0Resolved {
		return T2{}, &BehaviorError{Code: "UNKNOWN_COMPONENT", Message: "component 'Select' has no handler (fail-closed)"}
	}
	if row_n0.IsError {
		return T2{}, &BehaviorError{Code: "OP_FAILED", Message: "operation 'n0' failed under 'fail' policy: "+row_n0.Err}
	}
	t_n0 = row_n0.Val
	produced_n0 = true
	// ── map 'rel_posts' (Select, batched, into:null, relationKind:connection) ──
	over_rel_posts := t_n0
	t_rel_posts = make([][]T1, 0, len(over_rel_posts))
	items_rel_posts := make([]PortsNR_FindFirst_rel_posts, 0, len(over_rel_posts))
	for _, oel_rel_posts := range over_rel_posts {
		ep_rel_posts := PortsNR_FindFirst_rel_posts{Sql: "SELECT id, title, author_id FROM benchmark_posts WHERE author_id IN (SELECT value FROM json_each(?)) ORDER BY id ASC", K0: oel_rel_posts.Id}
		items_rel_posts = append(items_rel_posts, ep_rel_posts)
	}
	if len(items_rel_posts) > 0 {
		bp_rel_posts := PortsNR_FindFirst_rel_posts_batch{Items: items_rel_posts}
		mo_rel_posts, mo_rel_postsResolved := handlers.Node_FindFirst_rel_posts(bp_rel_posts, nil)
		if !mo_rel_postsResolved {
			return T2{}, &BehaviorError{Code: "UNKNOWN_COMPONENT", Message: "component 'Select' has no handler (fail-closed)"}
		}
		if mo_rel_posts.IsError {
			return T2{}, &BehaviorError{Code: "OP_FAILED", Message: "operation 'rel_posts' failed under 'fail' policy: "+mo_rel_posts.Err}
		}
		if len(mo_rel_posts.Rows) != len(items_rel_posts) {
			return T2{}, &BehaviorError{Code: "MAP_BATCH_RESULT_MISMATCH", Message: fmt.Sprintf("map 'rel_posts': batched handler must return a list aligned to items (want %d)", len(items_rel_posts))}
		}
		for mk_rel_posts := range mo_rel_posts.Rows {
			var el_rel_posts []T1
			el_rel_posts = mo_rel_posts.Rows[mk_rel_posts].Val
			t_rel_posts = append(t_rel_posts, el_rel_posts)
		}
	}
	produced_rel_posts = true
	return T2{Rows: t_n0, Posts: t_rel_posts}, nil
}

// ComponentNamesNativeRaw — covered reads exposed on the combined struct-native path (declaration
// order). Each is driven via RunNativeRawStruct_<comp>(handlers, input) -> (T, error): a STRUCT
// return; the consumer keeps the model native (no Value serialization on the read hot path — the
// boxing residue bc#77 removes). See INTEGRATION.md §6.
var ComponentNamesNativeRaw = []string{"FindFirst"}
