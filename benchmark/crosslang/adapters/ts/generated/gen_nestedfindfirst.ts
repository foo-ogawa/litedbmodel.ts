...
export const COMPONENT_NAMES: readonly string[] = ["FindFirst"];
...
function run_FindFirst(h$Select: Handler | undefined, input: Scope): Value {
  ...
  const ports_n0: Record<string, Value> = {
    "sql": "SELECT id, email, name FROM benchmark_users WHERE name LIKE ? LIMIT ?",
...
async function run_FindFirst_async(h$Select: AsyncHandler | undefined, input: Scope): Promise<Value> {
  const scope: Scope = input;
  // ── op 'n0' (Select) ──
  if (!h$Select) throw new BehaviorFailure("UNKNOWN_COMPONENT", "component 'Select' has no handler (fail-closed)");
  const ports_n0: Record<string, Value> = {
...
function bind_FindFirst(handlers: Handlers): (input?: Scope) => Value {
  ...
  return (input: Scope = {}) => run_FindFirst(h$Select, { ...input });
}

function bindAsync_FindFirst(handlers: AsyncHandlers): (input?: Scope) => Promise<Value> {
  const h$Select = handlers["Select"];
  return (input: Scope = {}) => run_FindFirst_async(h$Select, { ...input });
}
...
export function typedView_FindFirst(scope: Record<string, Value>): T2 {
