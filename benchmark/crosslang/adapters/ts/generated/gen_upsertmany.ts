...
function run_UpsertMany(h$Insert: Handler | undefined, input: Scope): Value {
  ...
  const ports_n0: Record<string, Value> = {
    "sql": "INSERT INTO benchmark_users (email, name) SELECT json_extract(value, '$.email'), json_extract(value, '$.name') FROM json_each(?) WHERE true ON CONFLICT (email) DO UPDATE SET email = excluded.email, name = excluded.name RETURNING id, email, name",
