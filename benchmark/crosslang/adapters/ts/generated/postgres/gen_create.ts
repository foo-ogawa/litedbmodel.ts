...
function run_Create(h$Insert: Handler | undefined, input: Scope): Value {
  // ── op 'n0' (Insert) ──
  ...
  const ports_n0: Record<string, Value> = {
    "sql": "INSERT INTO benchmark_users (email, name) VALUES ($1, $2)",
    "p0": slBind(input, "email"),
    "p1": slBind(input, "name"),
  };
  const o_n0 = h$Insert(ports_n0, { nodeId: "n0", component: "Insert" });
...
async function run_Create_async(h$Insert: AsyncHandler | undefined, input: Scope): Promise<Value> {
  // ── op 'n0' (Insert) ──
  if (!h$Insert) throw new BehaviorFailure("UNKNOWN_COMPONENT", "component 'Insert' has no handler (fail-closed)");
  const ports_n0: Record<string, Value> = {
    "sql": "INSERT INTO benchmark_users (email, name) VALUES ($1, $2)",
  ...
  const o_n0 = await h$Insert(ports_n0, { nodeId: "n0", component: "Insert" });
  if ("error" in o_n0) throw new PlanFailure("OP_FAILED", `operation 'n0' failed under 'fail' policy: ${o_n0.error}`);
  const r_n0: Value = conformResultToOutType("n0", o_n0.ok, {"arr":{"obj":{"changes":"int","lastInsertRowid":"int"}}}, false);
  return r_n0;
}
...
function bind_Create(handlers: Handlers): (input?: Scope) => Value {
  ...
  return (input: Scope = {}) => run_Create(h$Insert, { ...input });
}

function bindAsync_Create(handlers: AsyncHandlers): (input?: Scope) => Promise<Value> {
  const h$Insert = handlers["Insert"];
  return (input: Scope = {}) => run_Create_async(h$Insert, { ...input });
}
...
export function typedView_Create(scope: Record<string, Value>): T0[] {
