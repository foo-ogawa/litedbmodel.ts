...
export const COMPONENT_NAMES: readonly string[] = ["CreateMany"];
...
function run_CreateMany(h$Insert: Handler | undefined, input: Scope): Value {
  // ── op 'n0' (Insert) ──
  ...
  const ports_n0: Record<string, Value> = {
    "sql": "INSERT INTO benchmark_users (email, name) SELECT v.email, v.name FROM UNNEST($1::text[], $2::text[]) AS v(email, name)",
...
async function run_CreateMany_async(h$Insert: AsyncHandler | undefined, input: Scope): Promise<Value> {
  // ── op 'n0' (Insert) ──
  if (!h$Insert) throw new BehaviorFailure("UNKNOWN_COMPONENT", "component 'Insert' has no handler (fail-closed)");
  const ports_n0: Record<string, Value> = {
    "sql": "INSERT INTO benchmark_users (email, name) SELECT v.email, v.name FROM UNNEST($1::text[], $2::text[]) AS v(email, name)",
...
function bind_CreateMany(handlers: Handlers): (input?: Scope) => Value {
  ...
  return (input: Scope = {}) => run_CreateMany(h$Insert, { ...input });
}

function bindAsync_CreateMany(handlers: AsyncHandlers): (input?: Scope) => Promise<Value> {
  const h$Insert = handlers["Insert"];
  return (input: Scope = {}) => run_CreateMany_async(h$Insert, { ...input });
}
...
export function typedView_CreateMany(scope: Record<string, Value>): T0[] {
