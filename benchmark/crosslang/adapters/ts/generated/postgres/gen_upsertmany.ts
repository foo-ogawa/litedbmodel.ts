...
export const COMPONENT_NAMES: readonly string[] = ["UpsertMany"];
...
function run_UpsertMany(h$Insert: Handler | undefined, input: Scope): Value {
  // ── op 'n0' (Insert) ──
  ...
  const ports_n0: Record<string, Value> = {
    "sql": "INSERT INTO benchmark_users (email, name) SELECT v.email, v.name FROM UNNEST($1::text[], $2::text[]) AS v(email, name) ON CONFLICT (email) DO UPDATE SET email = EXCLUDED.email, name = EXCLUDED.name",
    "v0": slBind(input, "emails"),
...
async function run_UpsertMany_async(h$Insert: AsyncHandler | undefined, input: Scope): Promise<Value> {
  // ── op 'n0' (Insert) ──
  if (!h$Insert) throw new BehaviorFailure("UNKNOWN_COMPONENT", "component 'Insert' has no handler (fail-closed)");
  const ports_n0: Record<string, Value> = {
    "sql": "INSERT INTO benchmark_users (email, name) SELECT v.email, v.name FROM UNNEST($1::text[], $2::text[]) AS v(email, name) ON CONFLICT (email) DO UPDATE SET email = EXCLUDED.email, name = EXCLUDED.name",
...
function bind_UpsertMany(handlers: Handlers): (input?: Scope) => Value {
  ...
  return (input: Scope = {}) => run_UpsertMany(h$Insert, { ...input });
}

function bindAsync_UpsertMany(handlers: AsyncHandlers): (input?: Scope) => Promise<Value> {
  const h$Insert = handlers["Insert"];
  return (input: Scope = {}) => run_UpsertMany_async(h$Insert, { ...input });
}
...
export function typedView_UpsertMany(scope: Record<string, Value>): T0[] {
