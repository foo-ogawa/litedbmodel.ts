...
export const COMPONENT_NAMES: readonly string[] = ["Update"];
...
function run_Update(h$Update: Handler | undefined, input: Scope): Value {
  // ── op 'n0' (Update) ──
...
async function run_Update_async(h$Update: AsyncHandler | undefined, input: Scope): Promise<Value> {
  // ── op 'n0' (Update) ──
  if (!h$Update) throw new BehaviorFailure("UNKNOWN_COMPONENT", "component 'Update' has no handler (fail-closed)");
  const ports_n0: Record<string, Value> = {
    "sql": "UPDATE benchmark_users SET name = $1 WHERE id = $2",
...
function bind_Update(handlers: Handlers): (input?: Scope) => Value {
  ...
  return (input: Scope = {}) => run_Update(h$Update, { ...input });
}

function bindAsync_Update(handlers: AsyncHandlers): (input?: Scope) => Promise<Value> {
  const h$Update = handlers["Update"];
  return (input: Scope = {}) => run_Update_async(h$Update, { ...input });
}
...
export function typedView_Update(scope: Record<string, Value>): T0[] {
