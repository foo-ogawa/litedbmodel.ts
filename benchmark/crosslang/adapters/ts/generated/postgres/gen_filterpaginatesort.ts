...
export const COMPONENT_NAMES: readonly string[] = ["FilterPaginateSort"];
...
function run_FilterPaginateSort(h$Select: Handler | undefined, input: Scope): Value {
  // ── op 'n0' (Select) ──
  ...
  const ports_n0: Record<string, Value> = {
    "sql": "SELECT id, title, content, published, author_id, created_at FROM benchmark_posts WHERE published = $1 ORDER BY created_at DESC LIMIT $2 OFFSET $3",
    "p0": slBind(input, "published"),
    "p1": cgp.numberLit(20),
    "p2": cgp.numberLit(10),
  };
  ...
  const r_n0: Value = conformResultToOutType("n0", o_n0.ok, {"arr":{"obj":{"id":"int","title":"string","content":"string","published":"bool","author_id":"int","created_at":"string"}}}, false);
  return r_n0;
}

async function run_FilterPaginateSort_async(h$Select: AsyncHandler | undefined, input: Scope): Promise<Value> {
  // ── op 'n0' (Select) ──
  if (!h$Select) throw new BehaviorFailure("UNKNOWN_COMPONENT", "component 'Select' has no handler (fail-closed)");
  const ports_n0: Record<string, Value> = {
    "sql": "SELECT id, title, content, published, author_id, created_at FROM benchmark_posts WHERE published = $1 ORDER BY created_at DESC LIMIT $2 OFFSET $3",
...
function bind_FilterPaginateSort(handlers: Handlers): (input?: Scope) => Value {
  ...
  return (input: Scope = {}) => run_FilterPaginateSort(h$Select, { ...input });
}

function bindAsync_FilterPaginateSort(handlers: AsyncHandlers): (input?: Scope) => Promise<Value> {
  const h$Select = handlers["Select"];
  return (input: Scope = {}) => run_FilterPaginateSort_async(h$Select, { ...input });
}
...
export function typedView_FilterPaginateSort(scope: Record<string, Value>): T0[] {
