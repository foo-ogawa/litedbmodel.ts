...
export const COMPONENT_NAMES: readonly string[] = ["NestedUpsert"];
...
function run_NestedUpsert(h$Insert: Handler | undefined, input: Scope): Value {
  ...
  const ports_tx_body_0: Record<string, Value> = {
    "sql": "INSERT INTO benchmark_users (email, name) VALUES ($1, $2) ON CONFLICT (email) DO UPDATE SET email = excluded.email, name = excluded.name RETURNING id",
...
async function run_NestedUpsert_async(h$Insert: AsyncHandler | undefined, input: Scope): Promise<Value> {
  const scope: Scope = input;
  // ── op 'tx_body_0' (Insert) ──
  if (!h$Insert) throw new BehaviorFailure("UNKNOWN_COMPONENT", "component 'Insert' has no handler (fail-closed)");
  const ports_tx_body_0: Record<string, Value> = {
...
function bind_NestedUpsert(handlers: Handlers): (input?: Scope) => Value {
  ...
  return (input: Scope = {}) => run_NestedUpsert(h$Insert, { ...input });
}

function bindAsync_NestedUpsert(handlers: AsyncHandlers): (input?: Scope) => Promise<Value> {
  const h$Insert = handlers["Insert"];
  return (input: Scope = {}) => run_NestedUpsert_async(h$Insert, { ...input });
}
...
export function typedView_NestedUpsert(scope: Record<string, Value>): T2 {
