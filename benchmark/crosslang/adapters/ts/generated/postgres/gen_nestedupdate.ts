...
function run_NestedUpdate(h$Update: Handler | undefined, input: Scope): Value {
  ...
  const ports_tx_body_0: Record<string, Value> = {
    "sql": "UPDATE benchmark_users SET name = $1 WHERE id = $2",
  ...
  const r_tx_body_0: Value = conformResultToOutType("tx_body_0", o_tx_body_0.ok, {"obj":{"changes":"int","lastInsertRowid":"int"}}, false);
  scope["tx_body_0"] = r_tx_body_0;
  // ── op 'tx_body_1' (Update) ──
  if (!h$Update) throw new BehaviorFailure("UNKNOWN_COMPONENT", "component 'Update' has no handler (fail-closed)");
  const ports_tx_body_1: Record<string, Value> = {
    "sql": "UPDATE benchmark_posts SET title = $1 WHERE author_id = $2",
...
export function typedView_NestedUpdate(scope: Record<string, Value>): T1 {
  const t_tx_body_0: T0 = scope["tx_body_0"] as unknown as T0;
  const t_tx_body_1: T0 = scope["tx_body_1"] as unknown as T0;
  const __out: T1 = { "user": t_tx_body_0, "post": t_tx_body_1 };
  return __out;
}
