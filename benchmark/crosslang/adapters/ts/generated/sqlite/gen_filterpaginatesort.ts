...
function run_FilterPaginateSort(h$Select: Handler | undefined, input: Scope): Value {
  ...
  const ports_n0: Record<string, Value> = {
    "sql": "SELECT id, title, content, published, author_id, created_at FROM benchmark_posts WHERE published = ? ORDER BY created_at DESC LIMIT ? OFFSET ?",
  ...
  const r_n0: Value = conformResultToOutType("n0", o_n0.ok, {"arr":{"obj":{"id":"int","title":"string","content":"string","published":"int","author_id":"int","created_at":"string"}}}, false);
  return r_n0;
}
