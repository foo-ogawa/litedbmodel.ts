...
function run_UpdateMany(h$Update: Handler | undefined, input: Scope): Value {
  ...
  const ports_n0: Record<string, Value> = {
    "sql": "UPDATE benchmark_users SET name = (SELECT json_extract(je.value, '$.name') FROM json_each(?) je WHERE json_extract(je.value, '$.id') = benchmark_users.id LIMIT 1) WHERE id IN (SELECT json_extract(value, '$.id') FROM json_each(?)) RETURNING id, email, name",
    "v0": slBind(input, "ids"),
