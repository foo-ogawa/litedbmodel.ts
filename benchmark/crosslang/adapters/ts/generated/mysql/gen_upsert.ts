...
export const COMPONENT_NAMES: readonly string[] = ["Upsert"];
...
function run_Upsert(h$Insert: Handler | undefined, input: Scope): Value {
  // ── op 'n0' (Insert) ──
  ...
  const r_n0: Value = conformResultToOutType("n0", o_n0.ok, {"arr":{"obj":{"id":"int"}}}, false);
  return r_n0;
}

async function run_Upsert_async(h$Insert: AsyncHandler | undefined, input: Scope): Promise<Value> {
  // ── op 'n0' (Insert) ──
  if (!h$Insert) throw new BehaviorFailure("UNKNOWN_COMPONENT", "component 'Insert' has no handler (fail-closed)");
  const ports_n0: Record<string, Value> = {
    "sql": "INSERT INTO benchmark_users (email, name) VALUES (?, ?) ON DUPLICATE KEY UPDATE email = VALUES(email), name = VALUES(name) /*scp-reselect: SELECT id FROM benchmark_users WHERE email = ? ORDER BY id ::binds:: p0*/",
...
function bind_Upsert(handlers: Handlers): (input?: Scope) => Value {
  ...
  return (input: Scope = {}) => run_Upsert(h$Insert, { ...input });
}

function bindAsync_Upsert(handlers: AsyncHandlers): (input?: Scope) => Promise<Value> {
  const h$Insert = handlers["Insert"];
  return (input: Scope = {}) => run_Upsert_async(h$Insert, { ...input });
}
...
export function typedView_Upsert(scope: Record<string, Value>): T0[] {
