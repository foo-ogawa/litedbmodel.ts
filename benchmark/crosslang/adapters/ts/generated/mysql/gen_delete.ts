...
export const COMPONENT_NAMES: readonly string[] = ["Delete"];
...
function run_Delete(h$Insert: Handler | undefined, h$Delete: Handler | undefined, input: Scope): Value {
  ...
  // ── op 'tx_body_1' (Delete) ──
  if (!h$Delete) throw new BehaviorFailure("UNKNOWN_COMPONENT", "component 'Delete' has no handler (fail-closed)");
  const ports_tx_body_1: Record<string, Value> = {
    "sql": "DELETE FROM benchmark_users WHERE id = ?",
    "p0": slField("ref", r_tx_body_0, ["id"]),
  };
  const o_tx_body_1 = h$Delete(ports_tx_body_1, { nodeId: "tx_body_1", component: "Delete" });
  ...
  return cgp.obj([["user", () => cgp.ref(["tx_body_0"], scope)], ["deleted", () => cgp.ref(["tx_body_1"], scope)]]);
}

async function run_Delete_async(h$Insert: AsyncHandler | undefined, h$Delete: AsyncHandler | undefined, input: Scope): Promise<Value> {
  const scope: Scope = input;
  // ── op 'tx_body_0' (Insert) ──
  if (!h$Insert) throw new BehaviorFailure("UNKNOWN_COMPONENT", "component 'Insert' has no handler (fail-closed)");
  const ports_tx_body_0: Record<string, Value> = {
  ...
  const o_tx_body_1 = await h$Delete(ports_tx_body_1, { nodeId: "tx_body_1", component: "Delete" });
  if ("error" in o_tx_body_1) throw new PlanFailure("OP_FAILED", `operation 'tx_body_1' failed under 'fail' policy: ${o_tx_body_1.error}`);
  const r_tx_body_1: Value = conformResultToOutType("tx_body_1", o_tx_body_1.ok, {"obj":{"changes":"int","lastInsertRowid":"int"}}, false);
  scope["tx_body_1"] = r_tx_body_1;
  return cgp.obj([["user", () => cgp.ref(["tx_body_0"], scope)], ["deleted", () => cgp.ref(["tx_body_1"], scope)]]);
...
function bind_Delete(handlers: Handlers): (input?: Scope) => Value {
  ...
  const h$Delete = handlers["Delete"];
  return (input: Scope = {}) => run_Delete(h$Insert, h$Delete, { ...input });
}

function bindAsync_Delete(handlers: AsyncHandlers): (input?: Scope) => Promise<Value> {
  const h$Insert = handlers["Insert"];
  const h$Delete = handlers["Delete"];
  return (input: Scope = {}) => run_Delete_async(h$Insert, h$Delete, { ...input });
}
...
export function typedView_Delete(scope: Record<string, Value>): T2 {
  const t_tx_body_0: T0 = scope["tx_body_0"] as unknown as T0;
  const t_tx_body_1: T1 = scope["tx_body_1"] as unknown as T1;
  const __out: T2 = { "user": t_tx_body_0, "deleted": t_tx_body_1 };
  return __out;
}
