...
// Row_CreateMany_n0 — CONCRETE handler-result row (typed fields = the node's outType; + error signal).
type Row_CreateMany_n0 struct {
	IsError bool
	Err     string
	Val []T0
}
...
// Handler_CreateMany — the CONCRETE per-component handler seam: one typed method
...
type Handler_CreateMany interface {
	Node_CreateMany_n0(ports PortsNR_CreateMany_n0, bound *string) (Row_CreateMany_n0, bool)
}
...
func RunNativeRawStruct_CreateMany[H Handler_CreateMany](handlers H, in In_CreateMany) ([]T0, error) {
	_ = in
	var t_n0 []T0
	produced_n0 := false
	_ = t_n0
	_ = produced_n0
	// ── op 'n0' (Insert) ──
	ports_n0 := PortsNR_CreateMany_n0{Sql: "INSERT INTO benchmark_users (email, name) SELECT json_extract(value, '$.email'), json_extract(value, '$.name') FROM json_each(?) RETURNING id, email, name", V0: in.Emails, V1: in.Names}
	row_n0, row_n0Resolved := handlers.Node_CreateMany_n0(ports_n0, nil)
	if !row_n0Resolved {
