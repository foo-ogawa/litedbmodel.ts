...
// In_CreateUser — the CONCRETE input for 'CreateUser' (fields = inputPorts; typed, consumer-built —
// NO generic *Obj, NO per-field boxing crosses the covered read boundary).
type In_CreateUser struct {
	Email string // "email"
	Name  string // "name"
}

// CONCRETE per-node handler-result row structs (typed fields = the node's outType; + error signal).
// Row_CreateUser_n0 — CONCRETE handler-result row (typed fields = the node's outType; + error signal).
type Row_CreateUser_n0 struct {
	IsError bool
	Err     string
	Val []T0
}
...
// Handler_CreateUser — the CONCRETE per-component handler seam: one typed method
...
type Handler_CreateUser interface {
	Node_CreateUser_n0(ports PortsNR_CreateUser_n0, bound *string) (Row_CreateUser_n0, bool)
}

// Native ports structs (one per componentRef node; typed per the static port type).
// PortsNR_CreateUser_n0 — NATIVE ports for node 'n0' (Insert). Typed fields per the
...
type PortsNR_CreateUser_n0 struct {
	Sql string // "sql"
	P0 string // "p0"
	P1 string // "p1"
}
...
// RunNativeRawStruct_CreateUser — the STRUCT-RETURNING combined read (bc#77/#87): the fully
...
func RunNativeRawStruct_CreateUser[H Handler_CreateUser](handlers H, in In_CreateUser) ([]T0, error) {
	_ = in
	var t_n0 []T0
	produced_n0 := false
	_ = t_n0
	_ = produced_n0
	// ── op 'n0' (Insert) ──
	ports_n0 := PortsNR_CreateUser_n0{Sql: "INSERT INTO benchmark_users (email, name) VALUES (?, ?) RETURNING id, email, name", P0: in.Email, P1: in.Name}
	row_n0, row_n0Resolved := handlers.Node_CreateUser_n0(ports_n0, nil)
	if !row_n0Resolved {
		return nil, &BehaviorError{Code: "UNKNOWN_COMPONENT", Message: "component 'Insert' has no handler (fail-closed)"}
	}
	if row_n0.IsError {
...
var ComponentNamesNativeRaw = []string{"CreateUser"}
