...
// irFingerprint: fnv1a64:73aab41f049301f1
...
const IRFingerprint = "fnv1a64:73aab41f049301f1"

// ComponentNames are the components exposed by Bind(), in IR declaration order.
var ComponentNames = []string{"Posts"}
...
func run_Posts(handlers dslcontracts.ComponentExec, input *dslcontracts.Obj) (dslcontracts.Value, error) {
	if input == nil {
	...
	pv_n0___scope := slVE2(dslcontracts.PrimObj(dslcontracts.JObjOf("author_id", dslcontracts.JObjOf("ref", []dslcontracts.JNode{"author_id"})), scope))
...
func dispatch(name string, handlers dslcontracts.ComponentExec, input *dslcontracts.Obj) (dslcontracts.Value, error) {
	if name == "Posts" {
		return run_Posts(handlers, input)
	}
