...
pub const IR_FINGERPRINT: &str = "fnv1a64:5e989cf7594927a8";

/// Component names exposed by bind(), in IR declaration order.
pub const COMPONENT_NAMES: [&str; 1] = ["ByIds"];
...
fn run_ByIds<H: ComponentExec>(
    ...
    let eval_ports_n0 = || -> Result<Vec<(String, Value)>, behavior_contracts::ExprFailure> { Ok(vec![
        ("__scope".to_string(), (primitives::obj(&json!({ "ids": { "ref": ["ids"] } }), &scope))?),
    ]) };
...
impl<H: ComponentExec> Bound<H> {
    pub fn call(&mut self, name: &str, input: &[(String, Value)]) -> Result<Value, BehaviorError> {
        match name {
        "ByIds" => run_ByIds(&mut self.handlers, input),
