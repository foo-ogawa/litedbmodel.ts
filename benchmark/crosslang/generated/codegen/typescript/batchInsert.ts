 ...
 */
export const IR_FINGERPRINT = "fnv1a64:c3101fb11da847e5";

/** Component names exposed by bind(), in IR declaration order. */
export const COMPONENT_NAMES: readonly string[] = ["BatchInsert"];
...
function run_BatchInsert(h$makeSQL: Handler | undefined, input: Scope): Value {
  // ── op 'q' (makeSQL) ──
...
async function run_BatchInsert_async(h$makeSQL: AsyncHandler | undefined, input: Scope): Promise<Value> {
  // ── op 'q' (makeSQL) ──
  if (!h$makeSQL) throw new BehaviorFailure("UNKNOWN_COMPONENT", "component 'makeSQL' has no handler (fail-closed)");
  const ports_q: Record<string, Value> = {
    "sql": slBind(input, "__sql"),
...
function bind_BatchInsert(handlers: Handlers): (input?: Scope) => Value {
  // handler 解決は bind 時に 1 回（catalog 名 → 実装）。呼び出しは解決済み参照の直呼び。
  const h$makeSQL = handlers["makeSQL"];
  return (input: Scope = {}) => run_BatchInsert(h$makeSQL, { ...input });
}

function bindAsync_BatchInsert(handlers: AsyncHandlers): (input?: Scope) => Promise<Value> {
  const h$makeSQL = handlers["makeSQL"];
  return (input: Scope = {}) => run_BatchInsert_async(h$makeSQL, { ...input });
}
...
export function bind(handlers: Handlers): Record<string, (input?: Scope) => Value> {
  return {
    "BatchInsert": bind_BatchInsert(handlers),
  };
}
...
export function bindAsync(handlers: AsyncHandlers): Record<string, (input?: Scope) => Promise<Value>> {
  return {
    "BatchInsert": bindAsync_BatchInsert(handlers),
  };
}
