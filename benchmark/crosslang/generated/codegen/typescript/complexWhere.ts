 ...
 */
export const IR_FINGERPRINT = "fnv1a64:ead6ce5e6e15d33f";

/** Component names exposed by bind(), in IR declaration order. */
export const COMPONENT_NAMES: readonly string[] = ["ComplexWhere"];
...
function run_ComplexWhere(h$__makeSqlNode: Handler | undefined, input: Scope): Value {
...
async function run_ComplexWhere_async(h$__makeSqlNode: AsyncHandler | undefined, input: Scope): Promise<Value> {
  const scope: Scope = input;
  // ── op 'n0' (__makeSqlNode) ──
  if (!h$__makeSqlNode) throw new BehaviorFailure("UNKNOWN_COMPONENT", "component '__makeSqlNode' has no handler (fail-closed)");
  const ports_n0: Record<string, Value> = {
...
function bind_ComplexWhere(handlers: Handlers): (input?: Scope) => Value {
  // handler 解決は bind 時に 1 回（catalog 名 → 実装）。呼び出しは解決済み参照の直呼び。
  const h$__makeSqlNode = handlers["__makeSqlNode"];
  return (input: Scope = {}) => run_ComplexWhere(h$__makeSqlNode, { ...input });
}

function bindAsync_ComplexWhere(handlers: AsyncHandlers): (input?: Scope) => Promise<Value> {
  const h$__makeSqlNode = handlers["__makeSqlNode"];
  return (input: Scope = {}) => run_ComplexWhere_async(h$__makeSqlNode, { ...input });
}
...
export function bind(handlers: Handlers): Record<string, (input?: Scope) => Value> {
  return {
    "ComplexWhere": bind_ComplexWhere(handlers),
  };
}
...
export function bindAsync(handlers: AsyncHandlers): Record<string, (input?: Scope) => Promise<Value>> {
  return {
    "ComplexWhere": bindAsync_ComplexWhere(handlers),
  };
}
