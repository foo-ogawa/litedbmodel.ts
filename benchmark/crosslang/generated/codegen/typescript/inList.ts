 ...
 */
export const IR_FINGERPRINT = "fnv1a64:5e989cf7594927a8";

/** Component names exposed by bind(), in IR declaration order. */
export const COMPONENT_NAMES: readonly string[] = ["ByIds"];
...
function run_ByIds(h$__makeSqlNode: Handler | undefined, input: Scope): Value {
  ...
  const ports_n0: Record<string, Value> = {
    "__scope": cgp.obj({ "ids": { "ref": ["ids"] } }, scope),
  };
...
async function run_ByIds_async(h$__makeSqlNode: AsyncHandler | undefined, input: Scope): Promise<Value> {
  const scope: Scope = input;
  // ── op 'n0' (__makeSqlNode) ──
  if (!h$__makeSqlNode) throw new BehaviorFailure("UNKNOWN_COMPONENT", "component '__makeSqlNode' has no handler (fail-closed)");
  const ports_n0: Record<string, Value> = {
...
function bind_ByIds(handlers: Handlers): (input?: Scope) => Value {
  // handler 解決は bind 時に 1 回（catalog 名 → 実装）。呼び出しは解決済み参照の直呼び。
  const h$__makeSqlNode = handlers["__makeSqlNode"];
  return (input: Scope = {}) => run_ByIds(h$__makeSqlNode, { ...input });
}

function bindAsync_ByIds(handlers: AsyncHandlers): (input?: Scope) => Promise<Value> {
  const h$__makeSqlNode = handlers["__makeSqlNode"];
  return (input: Scope = {}) => run_ByIds_async(h$__makeSqlNode, { ...input });
}
...
export function bindAsync(handlers: AsyncHandlers): Record<string, (input?: Scope) => Promise<Value>> {
  return {
    "ByIds": bindAsync_ByIds(handlers),
  };
}
