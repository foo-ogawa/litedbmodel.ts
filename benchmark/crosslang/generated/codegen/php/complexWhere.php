...
$componentFns = [
    "ComplexWhere" => static function (array $handlers, array $input) use ($__P, $__sl_bind, $__sl_field, $__sl_cat, $__sl_ref, $__sl_ref_opt, $__sl_concat, $__bhFail, $__planFail): mixed {
        $scope = $input;
        // ── op 'n0' (__makeSqlNode) ──
        $ports_n0 = [
            "__scope" => $__P::obj((object) ["author_id" => (object) ["ref" => ["author_id"]], "since" => (object) ["ref" => ["since"]], "titleLike" => (object) ["ref" => ["titleLike"]], "ids" => (object) ["ref" => ["ids"]]], $scope),
        ];
        if (!isset($handlers["__makeSqlNode"])) {
            $__bhFail::raise('UNKNOWN_COMPONENT', "component '__makeSqlNode' has no handler (fail-closed)");
        }
        $h_n0 = $handlers["__makeSqlNode"];
        $o_n0 = $h_n0($ports_n0, ['nodeId' => "n0", 'component' => "__makeSqlNode"]);
        if (array_key_exists('error', $o_n0)) {
            $__planFail::raise('OP_FAILED', "operation 'n0' failed under 'fail' policy: " . $o_n0['error']);
        }
        $r_n0 = $o_n0['ok'];
        $scope["n0"] = $r_n0;
        return $r_n0;
    ...
    'bind' => static function (array $handlers) use ($componentNames, $componentFns, $__bhFail): array {
        ...
        foreach ($componentNames as $name) {
            $bound[$name] = static function (array $input = []) use ($name, $handlers, $componentFns, $__bhFail): mixed {
                if ($name === "ComplexWhere") return ($componentFns["ComplexWhere"])($handlers, $input);
