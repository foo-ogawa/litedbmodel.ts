...
IR_FINGERPRINT = "fnv1a64:c3101fb11da847e5"

# Component names exposed by bind(), in IR declaration order.
COMPONENT_NAMES = ("BatchInsert",)
...
def run_BatchInsert(handlers, input):
    # ── op 'q' (makeSQL) ──
    ports_q = {
        "sql": _sl_bind(input, "__sql"),
        "params": _sl_bind(input, "__sqlParams"),
        "skip": _sl_bind(input, "__skip"),
    }
    h_q = handlers.get("makeSQL")
    if h_q is None:
        raise BehaviorFailure("UNKNOWN_COMPONENT", "component 'makeSQL' has no handler (fail-closed)")
    o_q = h_q(ports_q, {"nodeId": "q", "component": "makeSQL"})
    if "error" in o_q:
        raise PlanFailure("OP_FAILED", "operation 'q' failed under 'fail' policy: %s" % (o_q["error"],))
    r_q = o_q["ok"]
    return r_q
...
def _dispatch(name, handlers, input):
    if name == "BatchInsert":
        return run_BatchInsert(handlers, dict(input))
