...
import { generateCodegenArtifact, generateRustCompanion } from '../../dist/scp/index.cjs';
...
async function main() {
  ...
  // allow(non_snake_case)). Emitted so the crate never hand-maintains the module list (drift-free).
  const modLines = manifest.flatMap((op) => [`pub mod generated_${op.id};`, `pub mod companion_${op.id};`]);
